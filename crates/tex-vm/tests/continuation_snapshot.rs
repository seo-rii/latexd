...
#[test]
fn input_exit_snapshot_replays_column_layout_and_class_projection() {
...
\end{document}";
    ...
    let document_ir = build_document_ir(&stream, &());
    let document_class = document_ir.document_class.expect("document class");
    let layout = document_ir.layout.expect("column layout");

    assert_eq!(document_class.name, "article");
    assert_eq!(document_class.options, ["twocolumn"]);
...
#[test]
fn input_exit_snapshot_replays_bibliography_identifier_wrappers() {
...
\end{document}"#;
    ...
    let item = expected
        ...
        .expect("bibliography item");
    assert_eq!(item.text, "10.1000/example arXiv:2401.00001");
}
...
#[test]
fn input_exit_snapshot_replays_phantom_wrapper_aliases() {
    let source = r#"\def\hide#1{\phantom{#1}}
\let\widehide\hphantom
\begin{document}
\input{barrier}
\begin{thebibliography}{1}
\bibitem{key}Visible \hide{Ghost}\widehide{Wide}\vphantom{Tall}Text.
\end{thebibliography}
\end{document}"#;
    let (expected, actual) = replay_render_events_after_input_exit(source);
    ...
    let item = expected
        ...
        .expect("bibliography item");
    assert_eq!(item.text, "Visible Text.");
}
