...
fn multipage_pdf_bytes(pages: &[(u32, u32)]) -> Vec<u8> {
    let mut objects = vec![
        "1 0 obj << /Type /Catalog /Pages 2 0 R >> endobj\n".to_string(),
        String::new(),
...
#[test]
fn operatornamewithlimits_math_normalizes_through_ir_and_display_list() {
    let source = r"\begin{document}Choose $\operatornamewithlimits{arg\,max}_{x \in S} f(x)$ now.\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let math_event = capture
        .events
        .events
    ...
    assert_eq!(
        math_event.raw_source,
        r"\operatornamewithlimits{arg\,max}_{x \in S} f(x)"
    );
    assert_eq!(
        math_event.normalized_text.as_deref(),
        Some("arg max_x in S f(x)")
    ...
    for hidden in [r"\operatornamewithlimits", r"\,", "{arg", "max}"] {
        assert!(!extracted_text.contains(hidden), "{extracted_text}");
        assert!(!display_list_text.contains(hidden), "{display_list_text}");
    }
}
...
#[test]
fn mathop_wrappers_normalize_through_ir_and_display_list() {
    let source = r"\begin{document}Choose $\mathop{\mathrm{arg\,max}}\limits_{x \in S} f(x)$ now.\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let math_event = capture
        .events
        .events
    ...
    assert_eq!(
        math_event.raw_source,
        r"\mathop{\mathrm{arg\,max}}\limits_{x \in S} f(x)"
    );
    assert_eq!(
        math_event.normalized_text.as_deref(),
        Some("arg max_x in S f(x)")
    ...
    for hidden in [r"\mathop", r"\mathrm", r"\limits", r"\,"] {
        assert!(!extracted_text.contains(hidden), "{extracted_text}");
        assert!(!display_list_text.contains(hidden), "{display_list_text}");
    }
}
...
#[test]
fn boxed_math_wrapper_normalizes_inner_math_through_display_list() {
    let source = r"\begin{document}Energy $E=\boxed{\frac{1}{2}}$ now.\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let math_event = capture
        .events
        .events
    ...
    let display_list_math_run = capture.page_display_lists[0]
        ...
        .iter()
        .find_map(|op| match op {
            DrawOp::TextRun(run) if run.text == "E = 1/2" => Some(run),
            _ => None,
        })
        .expect("display-list math text run");
    let extracted_text = capture.document_ir.extracted_text();
    ...
    assert_eq!(math_event.raw_source, r"E=\boxed{\frac{1}{2}}");
    assert_eq!(math_event.normalized_text.as_deref(), Some("E = 1/2"));
    assert_eq!(display_list_math_run.text, "E = 1/2");
    assert!(extracted_text.contains("Energy E = 1/2 now."));
    for visible in ["Energy", "E = 1/2", "now."] {
        assert!(display_list_text.contains(visible), "{display_list_text}");
    }
    for hidden in [r"\boxed", r"\frac", "{1}", "{2}"] {
        assert!(!extracted_text.contains(hidden), "{extracted_text}");
        assert!(!display_list_text.contains(hidden), "{display_list_text}");
    }
}
...
#[test]
fn high_dot_math_accents_normalize_through_ir_and_display_list() {
    let source = r"\begin{document}State $\dddot{x}+\ddddot{y}$ now.\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let math_event = capture
        .events
        .events
    ...
    let display_list_text = capture.page_display_lists[0]
        ...
        .collect::<Vec<_>>()
        .join(" ");
    let expected = "dddot(x) + ddddot(y)";

    assert_eq!(math_event.raw_source, r"\dddot{x}+\ddddot{y}");
    assert_eq!(math_event.normalized_text.as_deref(), Some(expected));
    assert!(extracted_text.contains("State dddot(x) + ddddot(y) now."));
    assert!(display_list_text.contains(expected), "{display_list_text}");
    for hidden in [r"\dddot", r"\ddddot"] {
        assert!(!extracted_text.contains(hidden), "{extracted_text}");
        assert!(!display_list_text.contains(hidden), "{display_list_text}");
    }
}
...
#[test]
fn cancel_math_wrappers_normalize_through_ir_and_display_list() {
    let source = r"\begin{document}State $\cancel{x}+\bcancel{y}+\xcancel{z}+\cancelto{0}{w}$ now.\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let math_event = capture
        .events
        .events
    ...
    let display_list_text = capture.page_display_lists[0]
        ...
        .collect::<Vec<_>>()
        .join(" ");
    let expected = "cancel(x) + bcancel(y) + xcancel(z) + cancelto(0, w)";

    assert_eq!(
        math_event.raw_source,
        r"\cancel{x}+\bcancel{y}+\xcancel{z}+\cancelto{0}{w}"
    );
    assert_eq!(math_event.normalized_text.as_deref(), Some(expected));
    assert!(
        extracted_text.contains("State cancel(x) + bcancel(y) + xcancel(z) + cancelto(0, w) now.")
    );
    assert!(display_list_text.contains(expected), "{display_list_text}");
    for hidden in [r"\cancel", r"\bcancel", r"\xcancel", r"\cancelto"] {
        assert!(!extracted_text.contains(hidden), "{extracted_text}");
        assert!(!display_list_text.contains(hidden), "{display_list_text}");
    }
}
...
#[test]
fn math_phantom_and_overlap_wrappers_normalize_through_ir_and_display_list() {
    let source = r"\begin{document}State $x+\smash{y}+\mathclap{z}+\phantom{ghost}+\hphantom{wide}+\vphantom{tall}+w$ now.\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let math_event = capture
        .events
        .events
    ...
    assert_eq!(
        math_event.raw_source,
        r"x+\smash{y}+\mathclap{z}+\phantom{ghost}+\hphantom{wide}+\vphantom{tall}+w"
    );
    assert_eq!(math_event.normalized_text.as_deref(), Some(expected));
    assert!(extracted_text.contains("State x + y + z + + + + w now."));
    assert!(display_list_text.contains(expected), "{display_list_text}");
    for hidden in [
        r"\smash",
        r"\mathclap",
...
fn prescript_and_tensor_math_wrappers_normalize_through_display_list() {
    ...
    let display_list_text = capture.page_display_lists[0]
        ...
        .collect::<Vec<_>>()
        .join(" ");
    let expected = "prescript(14, 2, C) + tensor(T, ^a_b)";

    assert_eq!(
        math_event.raw_source,
        r"\prescript{14}{2}{C}+\tensor{T}{^a_b}"
    );
    assert_eq!(math_event.normalized_text.as_deref(), Some(expected));
    assert!(extracted_text.contains("State prescript(14, 2, C) + tensor(T, ^a_b) now."));
    assert!(display_list_text.contains(expected), "{display_list_text}");
    for hidden in [r"\prescript", r"\tensor", "{14}", "{2}", "{T}"] {
        assert!(!extracted_text.contains(hidden), "{extracted_text}");
        assert!(!display_list_text.contains(hidden), "{display_list_text}");
    }
}
...
#[test]
fn basic_physics_math_wrappers_normalize_through_display_list() {
    let source = r"\begin{document}State $\abs*{x-y}+\dv{f}{x}+\pdv{u}{t}$ now.\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let math_event = capture
        .events
        .events
    ...
    let display_list_text = capture.page_display_lists[0]
        ...
        .collect::<Vec<_>>()
        .join(" ");
    let expected = "|x - y| + d f/d x + partial u/partial t";

    assert_eq!(math_event.raw_source, r"\abs*{x-y}+\dv{f}{x}+\pdv{u}{t}");
    assert_eq!(math_event.normalized_text.as_deref(), Some(expected));
    assert!(extracted_text.contains("State |x - y| + d f/d x + partial u/partial t now."));
    assert!(display_list_text.contains(expected), "{display_list_text}");
    for hidden in [
        r"\abs", r"\dv", r"\pdv", "{x-y}", "{f}", "{x}", "{u}", "{t}",
    ] {
        assert!(!extracted_text.contains(hidden), "{extracted_text}");
        assert!(!display_list_text.contains(hidden), "{display_list_text}");
    }
...
fn physics_braket_math_wrappers_normalize_through_display_list() {
    ...
    let display_list_text = capture.page_display_lists[0]
        ...
        .collect::<Vec<_>>()
        .join(" ");
    let expected = "[A, B] + {C, D} + <psi|H|psi> + <phi|O|psi>";

    assert_eq!(
        math_event.raw_source,
        r"\comm{A}{B}+\anticomm{C}{D}+\expval{H}{\psi}+\matrixel{\phi}{O}{\psi}"
    );
    assert_eq!(math_event.normalized_text.as_deref(), Some(expected));
    assert!(extracted_text.contains("State [A, B] + {C, D} + <psi|H|psi> + <phi|O|psi> now."));
    assert!(display_list_text.contains(expected), "{display_list_text}");
    for hidden in [
        r"\comm",
        r"\anticomm",
...
fn braced_modular_math_notation_normalizes_through_display_list() {
    ...
    let display_list_math_run = capture.page_display_lists[0]
        ...
        .iter()
        .find_map(|op| match op {
            DrawOp::TextRun(run) if run.text == "a (mod n) + b (m)" => Some(run),
            _ => None,
        })
        .expect("display-list math text run");
    let extracted_text = capture.document_ir.extracted_text();
    ...
    assert_eq!(math_event.raw_source, r"a\pmod{n}+b\pod{m}");
    assert_eq!(
        math_event.normalized_text.as_deref(),
        Some("a (mod n) + b (m)")
    );
    assert_eq!(display_list_math_run.text, "a (mod n) + b (m)");
    assert!(extracted_text.contains("Congruence a (mod n) + b (m) now."));
    for visible in ["Congruence", "a (mod n) + b (m)", "now."] {
        assert!(display_list_text.contains(visible), "{display_list_text}");
    }
    for hidden in [r"\pmod", r"\pod", "{n}", "{m}"] {
        assert!(!extracted_text.contains(hidden), "{extracted_text}");
        assert!(!display_list_text.contains(hidden), "{display_list_text}");
    }
}
...
#[test]
fn project_root_svg_vector_text_is_extractable_from_debug_pdf() {
    let tempdir = tempfile::tempdir().expect("tempdir");
    let root = Utf8PathBuf::from_path_buf(tempdir.path().to_path_buf()).expect("utf8 temp path");
    fs::create_dir_all(root.join("figures").as_std_path()).expect("create figures dir");
    fs::write(
        root.join("main.tex").as_std_path(),
        r"\begin{document}\includegraphics{figures/text.svg}\end{document}",
    )
    .expect("write source");
    fs::write(
        root.join("figures/text.svg").as_std_path(),
        br#"<svg width="2in" height="1in" viewBox="0 0 200 100"><text x="10" y="40" font-size="12">VectorText</text></svg>"#,
    )
    .expect("write svg");
    ...
    let image = capture.page_display_lists[0]
        ...
        .iter()
        .find_map(|op| match op {
            DrawOp::Image(image) if image.asset_ref == "figures/text.svg" => Some(image),
            _ => None,
        })
        .expect("svg image op");
    let pdf_text = String::from_utf8_lossy(&capture.display_list_pdf);
    ...
    assert_eq!(image.asset_format, Some(GraphicAssetFormat::Svg));
    assert!(image.diagnostic.is_none(), "{:?}", image.diagnostic);
    assert!(pdf_text.contains("(VectorText) Tj"), "{pdf_text}");
    ...
    let artifact_pdf_text = String::from_utf8_lossy(&artifact_pdf);
    assert!(
        artifact_pdf_text.contains("(VectorText) Tj"),
        "{artifact_pdf_text}"
    );

    let display_list_svg =
        fs::read_to_string(&paths.display_list_svgs[0]).expect("read display-list svg");
    assert!(display_list_svg.contains("data-image-asset-ref=\"figures/text.svg\""));
    assert!(display_list_svg.contains("data-image-asset-format=\"svg\""));
    assert!(display_list_svg.contains("data-image-embedded=\"true\""));
    assert!(display_list_svg.contains("href=\"data:image/svg+xml;charset=utf-8,%3Csvg"));
    assert!(!display_list_svg.contains("[image: figures/text.svg]"));
    ...
    let extracted_text = String::from_utf8_lossy(&output.stdout);
    assert!(extracted_text.contains("VectorText"), "{extracted_text}");
    assert!(
        !extracted_text.contains("figures/text.svg"),
        "{extracted_text}"
    );
}
...
#[test]
fn graphic_two_optional_bounding_box_syntax_drives_default_image_rect() {
    let capture = capture_internal_render_ir(
        "main.tex",
        r"\begin{document}\includegraphics[0pt,0pt][144pt,72pt]{figures/plot.pdf}\end{document}",
        &SemanticAux::default(),
    );
    let image = capture.page_display_lists[0]
        .ops
    ...
    let pdf_text = String::from_utf8_lossy(&capture.display_list_pdf);
    assert!(pdf_text.contains("[image: figures/plot.pdf]"));
    assert!(!pdf_text.contains("0pt,0pt"));
    assert!(!pdf_text.contains("144pt,72pt"));
}
...
#[test]
fn graphic_viewport_without_clip_offsets_project_root_artifact_images() {
    let tempdir = tempfile::tempdir().expect("tempdir");
    let root = Utf8PathBuf::from_path_buf(tempdir.path().to_path_buf()).expect("utf8 temp path");
    fs::create_dir_all(root.join("figures").as_std_path()).expect("create figures dir");
    fs::write(
    ...
    assert!(pdf_text.contains("q 2 0 0 2 71.5 718.5 cm /Im1 Do Q"));
...
#[test]
fn braced_comma_graphic_viewport_enables_project_root_crop_clipping() {
    let tempdir = tempfile::tempdir().expect("tempdir");
    let root = Utf8PathBuf::from_path_buf(tempdir.path().to_path_buf()).expect("utf8 temp path");
    fs::create_dir_all(root.join("figures").as_std_path()).expect("create figures dir");
    fs::write(
        root.join("main.tex").as_std_path(),
        r"\begin{document}\includegraphics[viewport={0.5pt,0.5pt,1.5pt,1.5pt},clip]{figures/plot.png}\end{document}",
    )
    .expect("write source");
    fs::write(
        root.join("figures/plot.png").as_std_path(),
    ...
    let display_list_svg =
        fs::read_to_string(&paths.display_list_svgs[0]).expect("read display-list svg");
    assert!(display_list_svg.contains("data-image-crop-viewport=\"0.5,0.5,1.5,1.5\""));
    assert!(display_list_svg.contains("clip-path=\"url(#image-clip-0)\""));
    assert!(display_list_svg.contains("data-image-crop-rendered=\"true\""));
    assert!(display_list_svg.contains("<image x=\"71.5\" y=\"71.5\" width=\"2\" height=\"2\""));
}
...
#[test]
fn project_root_render_ir_capture_converts_eps_assets_in_debug_artifacts_when_gs_available() {
    let Ok(_) = which::which("gs") else {
        return;
    };
    let tempdir = tempfile::tempdir().expect("tempdir");
    ...
    fs::write(
        root.join("main.tex").as_std_path(),
        r"\begin{document}\includegraphics{figures/vector.eps}\end{document}",
    )
    .expect("write source");
    fs::write(
        root.join("figures/vector.eps").as_std_path(),
...
",
    ...
    let image = capture.page_display_lists[0]
        ...
        .iter()
        .find_map(|op| match op {
            DrawOp::Image(image) if image.asset_ref == "figures/vector.eps" => Some(image),
            _ => None,
        })
        .expect("eps image op");
    let pdf_text = String::from_utf8_lossy(&capture.display_list_pdf);
    ...
    assert!(image.asset_hash.is_some());
    assert_eq!(image.asset_format, Some(GraphicAssetFormat::Eps));
    assert!(image.diagnostic.is_none());
    assert!(pdf_text.contains("/Subtype /Image"));
    assert!(pdf_text.contains("/Im1 Do"));
    assert!(!pdf_text.contains("[unsupported image: figures/vector.eps]"));
    ...
    let display_list_svg =
        fs::read_to_string(&paths.display_list_svgs[0]).expect("read display-list svg");
    assert!(display_list_svg.contains("data-image-asset-ref=\"figures/vector.eps\""));
    assert!(display_list_svg.contains("data-image-asset-format=\"eps\""));
    assert!(display_list_svg.contains("data-image-converted-format=\"png\""));
    assert!(display_list_svg.contains("data-image-embedded=\"true\""));
    assert!(display_list_svg.contains("href=\"data:image/png,%89PNG"));
    assert!(!display_list_svg.contains("[unsupported image: figures/vector.eps]"));
}

#[test]
fn project_root_unconvertible_eps_graphic_surfaces_render_artifact_placeholder() {
    let tempdir = tempfile::tempdir().expect("tempdir");
    let root = Utf8PathBuf::from_path_buf(tempdir.path().to_path_buf()).expect("utf8 temp path");
    fs::create_dir_all(root.join("figures").as_std_path()).expect("create figures dir");
    fs::write(
...
",
    ...
    assert!(svg.contains("data-image-asset-ref=\"figures/vector.eps\""));
    assert!(svg.contains("data-image-asset-format=\"eps\""));
    assert!(svg.contains("data-image-placeholder-kind=\"unsupported\""));
    assert!(svg.contains("[unsupported image: figures/vector.eps]"));
...
fn project_root_pdf_page_option_affects_converted_debug_asset() {
    ...
    fs::write(
        root.join("main.tex").as_std_path(),
        r"\begin{document}\includegraphics[page=2]{figures/multipage.pdf}\end{document}",
    )
    .expect("write source");
    fs::write(
    ...
    let display_list_svg =
        fs::read_to_string(&paths.display_list_svgs[0]).expect("read display-list svg");
    assert!(display_list_svg.contains("data-image-asset-ref=\"figures/multipage.pdf\""));
...
#[test]
fn braced_legacy_epsfig_paths_survive_ir_and_display_list() {
    let capture = capture_internal_render_ir_with_mounted_files(
        "main.tex",
        BRACED_LEGACY_EPSFIG_SOURCE,
        &SemanticAux::default(),
        &[("figures/plot.eps", "fake eps")],
    );
...
#[test]
fn floatrow_fcapside_preserves_image_and_caption_without_option_leakage() {
    let capture = capture_internal_render_ir_with_mounted_files(
        "main.tex",
        FLOATROW_FCAPSIDE_SOURCE,
        &SemanticAux::default(),
    ...
    assert!(
        capture.page_display_lists[0].ops.iter().any(
            |op| matches!(op, DrawOp::Image(image) if image.asset_ref == "figures/fcapside.pdf")
        )
    );

    let extracted_text = capture.document_ir.extracted_text();
    assert!(extracted_text.contains("Side float [?]."));
    for hidden in ["fcapside", "FBwidth", "key"] {
        assert!(!extracted_text.contains(hidden), "{extracted_text:?}");
    }
    ...
    let display_list_text = capture.page_display_lists[0]
        ...
        .collect::<Vec<_>>()
        .join("");
    assert!(display_list_text.contains("Side float [?]."));
    for hidden in ["fcapside", "FBwidth", "key"] {
        assert!(!display_list_text.contains(hidden), "{display_list_text:?}");
    }
}
...
#[test]
fn picinpar_window_environment_capture_survives_ir_without_option_leakage() {
    let capture =
    ...
    assert!(extracted_text.contains("Window table."));
    for hidden in [
        "figwindow",
        "tabwindow",
        "picinpar",
    ...
    assert!(
        capture.page_display_lists[0].ops.iter().any(
            |op| matches!(op, DrawOp::Image(image) if image.asset_ref == "figures/window.pdf")
        )
    );
}
...
fn math_substack_scripts_use_normalized_text_in_ir_and_display_list() {
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                == r"\sum_{\substack{i<j\\j<k}}^{n} x_i + \lim_{\substack{x\to0\\y\to0}} f(x,y)"
                && normalized_text.as_deref()
    ...
    assert!(
        normalized_display_list_text.contains(
            "sum_{substack(i < j; j < k)}^{n} x_i + lim_{substack(x -> 0; y -> 0)} f(x, y)"
        ),
        "{display_list_text}"
    );
    assert!(!display_list_text.contains(r"\substack"));
}
...
fn math_extended_arrows_use_normalized_text_in_ir_and_display_list() {
    ...
    let normalized_display_list_text = display_list_text
        ...
        .join(" ");
    assert!(
        normalized_display_list_text.contains("->^{p} X + Y <-^{d}_{n -> infinity} Z"),
        "{display_list_text}"
    );
    assert!(!display_list_text.contains(r"\xrightarrow"));
    assert!(!display_list_text.contains(r"\xleftarrow"));
}

#[test]
fn math_extended_arrow_variants_use_normalized_text_in_ir_and_display_list() {
    let source = r"\begin{document}Flow \(\xleftrightarrow{\phi} A + B \xRightarrow[n\to\infty]{p} C + D \xLeftarrow{q} E + F \xmapsto{f} G\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
        ...
        .expect("paragraph");
    let expected = "<->^{phi} A + B =>^{p}_{n -> infinity} C + D <=^{q} E + F ->^{f} G";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                == r"\xleftrightarrow{\phi} A + B \xRightarrow[n\to\infty]{p} C + D \xLeftarrow{q} E + F \xmapsto{f} G"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
...
fn math_leftward_arrow_aliases_use_normalized_text_in_ir_and_display_list() {
    ...
    let paragraph = capture
        ...
        .expect("paragraph");
    let expected = "A <- B + C <-hook D + E swarrow F + G nwarrow H";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                normalized_text,
                ..
            } if raw_source == r"A\longleftarrow B + C\hookleftarrow D + E\swarrow F + G\nwarrow H"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
...
#[test]
fn math_additional_arrow_variants_use_normalized_text_in_ir_and_display_list() {
    let source = r"\begin{document}Flow \(A\Uparrow B + C\Downarrow D + E\Updownarrow F + G\nrightarrow H + I\nleftarrow J + K\rightharpoonup L + M\leftharpoondown N\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
        ...
        .expect("paragraph");
    let expected = "A Uparrow B + C Downarrow D + E Updownarrow F + G not -> H + I not <- J + K rightharpoonup L + M leftharpoondown N";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                == r"A\Uparrow B + C\Downarrow D + E\Updownarrow F + G\nrightarrow H + I\nleftarrow J + K\rightharpoonup L + M\leftharpoondown N"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
...
#[test]
fn math_double_arrow_aliases_use_normalized_text_in_ir_and_display_list() {
    let source = r"\begin{document}Flow \(A\Lleftarrow B + C\Rrightarrow D\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
        ...
        .expect("paragraph");
    let expected = "A Lleftarrow B + C Rrightarrow D";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                normalized_text,
                ..
            } if raw_source == r"A\Lleftarrow B + C\Rrightarrow D"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
    ...
    for hidden in [r"\Lleftarrow", r"\Rrightarrow"] {
        assert!(!display_list_text.contains(hidden), "{display_list_text}");
    }
}
...
fn math_brace_groups_use_normalized_text_in_ir_and_display_list() {
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                normalized_text,
                ..
            } if raw_source == r"\overbrace{a+b}^{n} + \underbrace{c_d}_{\text{reason}}"
                && normalized_text.as_deref()
    ...
    let display_list_text = capture.page_display_lists[0]
        ...
        .collect::<Vec<_>>()
        .join("\n");
    assert!(display_list_text.contains("overbrace(a + b)^n + underbrace(c_d)_reason"));
    assert!(!display_list_text.contains(r"\overbrace"));
    assert!(!display_list_text.contains(r"\underbrace"));
}
...
#[test]
fn math_membership_aliases_and_negated_quantifiers_use_normalized_text_in_ir_and_display_list() {
    let source = r"\begin{document}Logic \(\nexists x + A\owns y + B\not\owns z\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
        ...
        .expect("paragraph");
    let expected = "not exists x + A contains y + B not contains z";
...
fn math_ellipsis_commands_use_normalized_text_in_ir_and_display_list() {
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                normalized_text,
                ..
            } if raw_source == r"x_1,\ldots,x_n + a_1+\cdots+a_n + b_1,\dots,b_k"
                && normalized_text.as_deref()
    ...
    let normalized_display_list_text = display_list_text
        ...
        .join(" ");
    assert!(
        normalized_display_list_text.contains("x_1, ..., x_n + a_1 + ... + a_n + b_1, ..., b_k"),
        "{display_list_text}"
    );
    assert!(!display_list_text.contains(r"\ldots"));
    assert!(!display_list_text.contains(r"\cdots"));
    assert!(!display_list_text.contains(r"\dots"));
}
...
fn math_relation_operators_use_normalized_text_in_ir_and_display_list() {
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                && normalized_text.as_deref()
                    == Some("a equiv b cong c simeq d propto e perp f parallel g << h >> i models J |- K -| L")
        )
    }));
    ...
    assert!(!display_list_text.contains(r"\equiv"));
    assert!(!display_list_text.contains(r"\parallel"));
    assert!(!display_list_text.contains(r"\vdash"));
}

#[test]
fn math_turnstile_relation_aliases_use_normalized_text_in_ir_and_display_list() {
    let source = r"\begin{document}Relations \(A\Vdash B + C\Vvdash D + E\vDash F + G\VDash H + I\nvdash J + K\nvDash L + M\nVdash N + O\nVDash P\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
        ...
        .expect("paragraph");
    let expected = "A ||- B + C |||- D + E |= F + G ||= H + I not |- J + K not |= L + M not ||- N + O not ||= P";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                == r"A\Vdash B + C\Vvdash D + E\vDash F + G\VDash H + I\nvdash J + K\nvDash L + M\nVdash N + O\nVDash P"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
    ...
    for hidden in [
        r"\Vdash", r"\Vvdash", r"\vDash", r"\VDash", r"\nvdash", r"\nvDash", r"\nVdash", r"\nVDash",
    ] {
        assert!(!display_list_text.contains(hidden), "{display_list_text}");
    }
}
...
#[test]
fn math_triangle_relation_aliases_use_normalized_text_in_ir_and_display_list() {
    let source = r"\begin{document}Relations \(A\vartriangleleft B + C\vartriangleright D + E\trianglelefteq F + G\trianglerighteq H + I\ntriangleleft J + K\ntriangleright L + M\ntrianglelefteq N + O\ntrianglerighteq P\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
        ...
        .expect("paragraph");
    let expected = "A vartriangleleft B + C vartriangleright D + E trianglelefteq F + G trianglerighteq H + I not triangleleft J + K not triangleright L + M not trianglelefteq N + O not trianglerighteq P";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                == r"A\vartriangleleft B + C\vartriangleright D + E\trianglelefteq F + G\trianglerighteq H + I\ntriangleleft J + K\ntriangleright L + M\ntrianglelefteq N + O\ntrianglerighteq P"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
...
fn math_relation_symbol_variants_use_normalized_text_in_ir_and_display_list() {
    ...
    let paragraph = capture
        ...
        .expect("paragraph");
    let expected = "a asymp b + c doteq d + e bowtie f + g smile h + i frown j";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                normalized_text,
                ..
            } if raw_source == r"a\asymp b + c\doteq d + e\bowtie f + g\smile h + i\frown j"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
    ...
    for hidden in [r"\asymp", r"\doteq", r"\bowtie", r"\smile", r"\frown"] {
        assert!(!display_list_text.contains(hidden), "{display_list_text}");
    }
}
...
#[test]
fn math_additional_relation_aliases_use_normalized_text_in_ir_and_display_list() {
    let source = r"\begin{document}Relations \(a\smallsmile b + c\smallfrown d + e\backcong f + g\curlyeqprec h + i\curlyeqsucc j\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
        ...
        .expect("paragraph");
    let expected =
        "a smallsmile b + c smallfrown d + e backcong f + g curlyeqprec h + i curlyeqsucc j";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                == r"a\smallsmile b + c\smallfrown d + e\backcong f + g\curlyeqprec h + i\curlyeqsucc j"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
...
fn math_order_relation_operators_use_normalized_text_in_ir_and_display_list() {
    ...
    assert!(
        normalized_display_list_text.contains("sqsubseteq K sqsupseteq L triangleq M"),
        "{display_list_text}"
    );
    assert!(!display_list_text.contains(r"\leqslant"));
    assert!(!display_list_text.contains(r"\preceq"));
    assert!(!display_list_text.contains(r"\triangleq"));
}

#[test]
fn math_extended_order_relations_use_normalized_text_in_ir_and_display_list() {
    let source = r"\begin{document}Order \(a\lessapprox b + c\gtrapprox d + e\precsim f + g\succsim h + i\preccurlyeq j + k\succcurlyeq l + m\nprec n + o\nsucc p + q\nsucceq r\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
        ...
        .expect("paragraph");
    let expected = "a lessapprox b + c gtrapprox d + e precsim f + g succsim h + i preccurlyeq j + k succcurlyeq l + m not prec n + o not succ p + q not succeq r";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                == r"a\lessapprox b + c\gtrapprox d + e\precsim f + g\succsim h + i\preccurlyeq j + k\succcurlyeq l + m\nprec n + o\nsucc p + q\nsucceq r"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
...
#[test]
fn math_prec_succ_approx_aliases_use_normalized_text_in_ir_and_display_list() {
    let source = r"\begin{document}Order \(a\precapprox b + c\succapprox d + e\nprecsim f + g\nsuccsim h + i\precnsim j + k\succnsim l + m\precnapprox n + o\succnapprox p\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
        ...
        .expect("paragraph");
    let expected = "a precapprox b + c succapprox d + e not precsim f + g not succsim h + i precnsim j + k succnsim l + m precnapprox n + o succnapprox p";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                == r"a\precapprox b + c\succapprox d + e\nprecsim f + g\nsuccsim h + i\precnsim j + k\succnsim l + m\precnapprox n + o\succnapprox p"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
...
#[test]
fn math_comparison_relation_aliases_use_normalized_text_in_ir_and_display_list() {
    let source =
        r"\begin{document}Order \(a\leqq b + c\geqq d + e\lessgtr f + g\gtrless h\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                normalized_text,
                ..
            } if raw_source == r"a\leqq b + c\geqq d + e\lessgtr f + g\gtrless h"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
    ...
    for hidden in [r"\leqq", r"\geqq", r"\lessgtr", r"\gtrless"] {
        assert!(!display_list_text.contains(hidden), "{display_list_text}");
    }
}

#[test]
fn math_negated_order_relation_aliases_use_normalized_text_in_ir_and_display_list() {
    let source = r"\begin{document}Order \(a\nleq b + c\ngeq d + e\nleqslant f + g\ngeqslant h + i\lneq j + k\gneq l + m\lneqq n + o\gneqq p\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
        ...
        .expect("paragraph");
    let expected = "a not <= b + c not >= d + e not <= f + g not >= h + i lneq j + k gneq l + m lneqq n + o gneqq p";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                == r"a\nleq b + c\ngeq d + e\nleqslant f + g\ngeqslant h + i\lneq j + k\gneq l + m\lneqq n + o\gneqq p"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
...
#[test]
fn math_negated_comparison_relation_aliases_use_normalized_text_in_ir_and_display_list() {
    let source = r"\begin{document}Order \(a\nless b + c\ngtr d + e\nleqq f + g\ngeqq h + i\lnsim j + k\gnsim l + m\lnapprox n + o\gnapprox p\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
        ...
        .expect("paragraph");
    let expected = "a not < b + c not > d + e not <= f + g not >= h + i lnsim j + k gnsim l + m lnapprox n + o gnapprox p";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                == r"a\nless b + c\ngtr d + e\nleqq f + g\ngeqq h + i\lnsim j + k\gnsim l + m\lnapprox n + o\gnapprox p"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
...
fn math_named_symbols_use_normalized_text_in_ir_and_display_list() {
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                normalized_text,
                ..
            } if raw_source == r"\ell + \aleph + \hbar + \Re z + \Im z"
                && normalized_text.as_deref()
    ...
    let normalized_display_list_text = display_list_text
        ...
        .join(" ");
    assert!(
        normalized_display_list_text.contains("ell + aleph + hbar + Re z + Im z"),
        "{display_list_text}"
    );
    assert!(!display_list_text.contains(r"\ell"));
    assert!(!display_list_text.contains(r"\aleph"));
    assert!(!display_list_text.contains(r"\hbar"));
}
...
#[test]
fn math_additional_named_symbol_aliases_use_normalized_text_in_ir_and_display_list() {
    let source = r"\begin{document}Symbols \(\imath + \jmath + \mathsection + \mathparagraph\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
        ...
        .expect("paragraph");
    let expected = "imath + jmath + mathsection + mathparagraph";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                normalized_text,
                ..
            } if raw_source == r"\imath + \jmath + \mathsection + \mathparagraph"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
    ...
    for hidden in [r"\imath", r"\jmath", r"\mathsection", r"\mathparagraph"] {
        assert!(!display_list_text.contains(hidden), "{display_list_text}");
    }
}
...
fn math_additional_special_symbols_use_normalized_text_in_ir_and_display_list() {
    ...
    let paragraph = capture
        ...
        .expect("paragraph");
    let expected = "eth + Finv + Game + backprime + diagup + diagdown + backepsilon";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                == r"\eth + \Finv + \Game + \backprime + \diagup + \diagdown + \backepsilon"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
...
#[test]
fn math_hslash_circled_and_currency_symbols_use_normalized_text_in_ir_and_display_list() {
    let source =
        r"\begin{document}Symbols \(\hslash + \circledR + \circledS + \yen\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
        ...
        .expect("paragraph");
    let expected = "hslash + circledR + circledS + yen";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                normalized_text,
                ..
            } if raw_source == r"\hslash + \circledR + \circledS + \yen"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
    ...
    for hidden in [r"\hslash", r"\circledR", r"\circledS", r"\yen"] {
        assert!(!display_list_text.contains(hidden), "{display_list_text}");
    }
}

#[test]
fn math_currency_and_copyright_symbols_use_normalized_text_in_ir_and_display_list() {
    let source = r"\begin{document}Symbols \(\pounds + \sterling + \mathsterling + \copyright\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
        ...
        .expect("paragraph");
    let expected = "pounds + sterling + mathsterling + copyright";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                normalized_text,
                ..
            } if raw_source == r"\pounds + \sterling + \mathsterling + \copyright"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
    ...
    for hidden in [r"\pounds", r"\sterling", r"\mathsterling", r"\copyright"] {
        assert!(!display_list_text.contains(hidden), "{display_list_text}");
    }
}

#[test]
fn math_geometry_and_suit_symbols_use_normalized_text_in_ir_and_display_list() {
    let source = r"\begin{document}Symbols \(\angle A + \measuredangle B + \sphericalangle C + \triangle + \square + \Box + \Diamond + \lozenge + \heartsuit + \spadesuit + \clubsuit + \diamondsuit\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
        ...
        .expect("paragraph");
    let expected = "angle A + measuredangle B + sphericalangle C + triangle + square + Box + Diamond + lozenge + heartsuit + spadesuit + clubsuit + diamondsuit";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                == r"\angle A + \measuredangle B + \sphericalangle C + \triangle + \square + \Box + \Diamond + \lozenge + \heartsuit + \spadesuit + \clubsuit + \diamondsuit"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
    ...
    let normalized_display_list_text = display_list_text
        ...
        .collect::<Vec<_>>()
        .join("");
    let expected_display_list_text = expected.split_whitespace().collect::<Vec<_>>().join("");
    assert!(
        normalized_display_list_text.contains(&expected_display_list_text),
        "{display_list_text}"
    );
    for hidden in [
        r"\angle",
...
#[test]
fn math_additional_triangle_symbols_use_normalized_text_in_ir_and_display_list() {
    let source = r"\begin{document}Symbols \(\vartriangle + \triangledown\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
        ...
        .expect("paragraph");
    let expected = "vartriangle + triangledown";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                normalized_text,
                ..
            } if raw_source == r"\vartriangle + \triangledown"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
    ...
    for hidden in [r"\vartriangle", r"\triangledown"] {
        assert!(!display_list_text.contains(hidden), "{display_list_text}");
    }
}

#[test]
fn math_black_symbol_aliases_use_normalized_text_in_ir_and_display_list() {
    let source = r"\begin{document}Symbols \(\blacktriangle + \blacktriangledown + \blacktriangleleft + \blacktriangleright + \blacksquare + \blacklozenge + \bigstar\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
        ...
        .expect("paragraph");
    let expected = "blacktriangle + blacktriangledown + blacktriangleleft + blacktriangleright + blacksquare + blacklozenge + bigstar";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                == r"\blacktriangle + \blacktriangledown + \blacktriangleleft + \blacktriangleright + \blacksquare + \blacklozenge + \bigstar"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
...
#[test]
fn math_music_and_marker_symbols_use_normalized_text_in_ir_and_display_list() {
    let source = r"\begin{document}Symbols \(\flat + \natural + \sharp + \checkmark + \maltese\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
        ...
        .expect("paragraph");
    let expected = "flat + natural + sharp + checkmark + maltese";
    ...
    for hidden in [r"\flat", r"\natural", r"\sharp", r"\checkmark", r"\maltese"] {
        assert!(!display_list_text.contains(hidden), "{display_list_text}");
    }
}

#[test]
fn math_logic_and_proof_symbols_use_normalized_text_in_ir_and_display_list() {
    let source =
        r"\begin{document}Logic \(\top + \bot + p \therefore q + r \because s\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
        ...
        .expect("paragraph");
    let expected = "top + bot + p therefore q + r because s";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                normalized_text,
                ..
            } if raw_source == r"\top + \bot + p \therefore q + r \because s"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
    ...
    for hidden in [r"\top", r"\bot", r"\therefore", r"\because"] {
        assert!(!display_list_text.contains(hidden), "{display_list_text}");
    }
}
...
#[test]
fn math_double_dagger_aliases_use_normalized_text_in_ir_and_display_list() {
    let source = r"\begin{document}Symbols \(A^\ddagger + B^\ddag\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                normalized_text,
                ..
            } if raw_source == r"A^\ddagger + B^\ddag"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
    ...
    for hidden in [r"\ddagger", r"\ddag"] {
        assert!(!display_list_text.contains(hidden), "{display_list_text}");
    }
}

#[test]
fn math_delimiter_and_punctuation_symbols_use_normalized_text_in_ir_and_display_list() {
    ...
    assert!(!display_list_text.contains(r"\lceil"));
    assert!(!display_list_text.contains(r"\colon"));
    assert!(!display_list_text.contains(r"\nleftrightarrow"));
}

#[test]
fn math_additional_delimiter_aliases_use_normalized_text_in_ir_and_display_list() {
    let source = r"\begin{document}Delims \(\lbrack x \rbrack + \lgroup y \rgroup + \arrowvert z \arrowvert + \Arrowvert w \Arrowvert\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                == r"\lbrack x \rbrack + \lgroup y \rgroup + \arrowvert z \arrowvert + \Arrowvert w \Arrowvert"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
...
#[test]
fn math_semantic_bracket_delimiters_use_normalized_text_in_ir_and_display_list() {
    let source = r"\begin{document}Semantics \(\llbracket P \rrbracket + \llbracket e \rrbracket_\rho\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                normalized_text,
                ..
            } if raw_source == r"\llbracket P \rrbracket + \llbracket e \rrbracket_\rho"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
    ...
    for hidden in [r"\llbracket", r"\rrbracket"] {
        assert!(!display_list_text.contains(hidden), "{display_list_text}");
    }
}
...
fn math_corner_delimiter_aliases_use_normalized_text_in_ir_and_display_list() {
    ...
    let paragraph = capture
        ...
        .expect("paragraph");
    let expected = "ulcorner A + B urcorner + llcorner C + D lrcorner";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                normalized_text,
                ..
            } if raw_source == r"\ulcorner A + B \urcorner + \llcorner C + D \lrcorner"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
    ...
    for hidden in [r"\ulcorner", r"\urcorner", r"\llcorner", r"\lrcorner"] {
        assert!(!display_list_text.contains(hidden), "{display_list_text}");
    }
}
...
#[test]
fn math_large_integral_operators_use_normalized_text_in_ir_and_display_list() {
    let source = r"\begin{document}Ops \(\oint_C f + \iint_D g + \iiint_{\Omega} h + \coprod_{i=1}^{n} A_i\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
        ...
        .expect("paragraph");
    let expected = "oint_{C} f + iint_{D} g + iiint_{Omega} h + coprod_{i = 1}^{n} A_i";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                normalized_text,
                ..
            } if raw_source == r"\oint_C f + \iint_D g + \iiint_{\Omega} h + \coprod_{i=1}^{n} A_i"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
    ...
    for hidden in [r"\oint", r"\iint", r"\iiint", r"\coprod"] {
        assert!(!display_list_text.contains(hidden), "{display_list_text}");
    }
}

#[test]
fn math_additional_integral_aliases_use_normalized_text_in_ir_and_display_list() {
    let source = r"\begin{document}Ops \(\smallint_0^1 f + \oiint_S g + \oiiint_V h + \ointclockwise_C k + \ointctrclockwise_D m\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
        ...
        .expect("paragraph");
    let expected = "smallint_{0}^{1} f + oiint_{S} g + oiiint_{V} h + ointclockwise_{C} k + ointctrclockwise_{D} m";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                == r"\smallint_0^1 f + \oiint_S g + \oiiint_V h + \ointclockwise_C k + \ointctrclockwise_D m"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
...
#[test]
fn math_additional_binary_operator_variants_use_normalized_text_in_ir_and_display_list() {
    let source = r"\begin{document}Ops \(A\sqcap B + C\triangleleft D + E\bigtriangleup F + G\bigtriangledown H + I\ltimes J + K\rtimes L + M\boxplus N + O\curlyvee P\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
        ...
        .expect("paragraph");
    let expected = "A sqcap B + C triangleleft D + E bigtriangleup F + G bigtriangledown H + I ltimes J + K rtimes L + M boxplus N + O curlyvee P";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                == r"A\sqcap B + C\triangleleft D + E\bigtriangleup F + G\bigtriangledown H + I\ltimes J + K\rtimes L + M\boxplus N + O\curlyvee P"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
...
#[test]
fn math_circled_and_barred_binary_operators_use_normalized_text_in_ir_and_display_list() {
    let source = r"\begin{document}Ops \(A\circledast B + C\circledcirc D + E\circleddash F + G\barwedge H + I\veebar J + K\doublebarwedge L\).\end{document}";
    let capture = capture_internal_render_ir("main.tex", source, &SemanticAux::default());
    let paragraph = capture
        .document_ir
        .blocks
        ...
        .expect("paragraph");
    let expected = "A circledast B + C circledcirc D + E circleddash F + G barwedge H + I veebar J + K doublebarwedge L";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                == r"A\circledast B + C\circledcirc D + E\circleddash F + G\barwedge H + I\veebar J + K\doublebarwedge L"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
...
fn math_set_and_composition_binary_operators_use_normalized_text_in_ir_and_display_list() {
    ...
    let paragraph = capture
        ...
        .expect("paragraph");
    let expected = "A Cup B + C Cap D + E dotplus F + G centerdot H + I interleave J + K doublecap L + M doublecup N";
    ...
    assert!(paragraph.content.iter().any(|node| {
        matches!(
            ...
            InlineNode::InlineMath {
                ...
                == r"A\Cup B + C\Cap D + E\dotplus F + G\centerdot H + I\interleave J + K\doublecap L + M\doublecup N"
                && normalized_text.as_deref() == Some(expected)
        )
    }));
...
#[test]
fn multiline_math_row_separators_use_normalized_text_in_ir_and_display_list() {
    ...
    assert!(capture.document_ir.blocks.iter().any(|block| {
        matches!(
            block,
            IrBlock::DisplayMath(display)
                if display.raw_source == r"a&=b\\c&=d"
                    && display.normalized_text.as_deref() == Some("a = b; c = d")
        )
    }));
    assert!(capture.document_ir.blocks.iter().any(|block| {
        matches!(
            block,
            IrBlock::DisplayMath(display)
                if display.raw_source == r"x=y\\z=w"
                    && display.normalized_text.as_deref() == Some("x = y; z = w")
        )
    }));
    ...
    assert!(display_list_text.contains("x = y; z = w"));
    assert!(!display_list_text.contains(r"a&=b\\c&=d"));
    assert!(!display_list_text.contains(r"x=y\\z=w"));
}

#[test]
fn nested_amsmath_environments_use_normalized_text_in_ir_and_display_list() {
    ...
    assert!(
        normalized_display_list_text
            .contains("split(a = b + c; = d) + gathered(x = y; z = w) + alignedat(p = q r = s)")
    );
    assert!(!display_list_text.contains(r"\begin{split}"));
    assert!(!display_list_text.contains(r"\begin{gathered}"));
    assert!(!display_list_text.contains(r"\begin{alignedat}"));
}
...
#[test]
fn tabular_extracolsep_hooks_do_not_become_visible_separators() {
    let capture = capture_internal_render_ir(
        "main.tex",
        r"\begin{document}\begin{tabular}{l@{\extracolsep{\fill}}r}A & 1 \\ B & 22\end{tabular}\end{document}",
        &SemanticAux::default(),
    );
    let table = capture
        .document_ir
    ...
    assert_eq!(table.columns.len(), 2);
    assert_eq!(table.columns[0].separator_after, None);
    let extracted_text = capture.document_ir.extracted_text();
    assert!(extracted_text.contains("A | 1"));
    assert!(extracted_text.contains("B | 22"));
    ...
    assert!(
        !table_lines
            .iter()
            .any(|line| line.contains("extracolsep") || line.contains("fill")),
        "{table_lines:?}"
    );
}
...
#[test]
fn tabular_phantom_wrappers_hide_invisible_text() {
    let capture = capture_internal_render_ir(
        "main.tex",
        r"\begin{document}\begin{tabular}{lll}Visible\phantom{Ghost} & Wide\hphantom{Hidden} & Tall\vphantom{Depth}\end{tabular}\end{document}",
        &SemanticAux::default(),
    );
    let table = capture
        .document_ir
    ...
    assert_eq!(table.rows[0].cells[1].text, "Wide");
    assert_eq!(table.rows[0].cells[2].text, "Tall");
    let extracted_text = capture.document_ir.extracted_text();
    assert!(extracted_text.contains("Visible | Wide | Tall"));
    ...
    let display_list_text = capture.page_display_lists[0]
        ...
        .collect::<Vec<_>>()
        .join("\n");
    assert!(display_list_text.contains("Visible | Wide | Tall"));
...
#[test]
fn tabularx_dimexpr_width_spec_stretches_display_list() {
    let capture = capture_internal_render_ir(
        "main.tex",
        r"\begin{document}\begin{tabularx}{\dimexpr\textwidth-36pt\relax}{lX}Alpha & Beta \\ Gamma & Delta\end{tabularx}\end{document}",
        &SemanticAux::default(),
    );
    let table = capture
        .document_ir
    ...
    assert_eq!(
        table.width_spec.as_deref(),
        Some(r"\dimexpr\textwidth-36pt\relax")
    );
    let table_lines = capture.page_display_lists[0]
        .ops
        .iter()
    ...
    let extracted_text = capture.document_ir.extracted_text();
    assert!(!extracted_text.contains("dimexpr"), "{extracted_text:?}");
}

#[test]
fn tabularstar_target_width_stretches_separator_without_trailing_padding() {
    let capture = capture_internal_render_ir(
        "main.tex",
        r"\begin{document}\begin{tabular*}{120pt}{lr}Alpha & Beta \\ Gamma & Delta\end{tabular*}\end{document}",
        &SemanticAux::default(),
    );
    let table = capture
        .document_ir
    ...
    assert_eq!(table.width_spec.as_deref(), Some("120pt"));
    let table_lines = capture.page_display_lists[0]
        .ops
        .iter()
        .filter_map(|op| match op {
    ...
    assert!(alpha_line.starts_with("Alpha | "), "{table_lines:?}");
...
#[test]
fn picture_fallback_uses_placeholder_in_ir_and_display_list() {
    let capture =
        capture_internal_render_ir("main.tex", PICTURE_FALLBACK_SOURCE, &SemanticAux::default());
    let fallback = capture
        .document_ir
        .blocks
        .iter()
        .find_map(|block| match block {
            IrBlock::RawFallback(fallback)
                if fallback.environment.as_deref() == Some("picture") =>
            {
                Some(fallback)
            }
            _ => None,
        })
        .expect("picture fallback");

    assert_eq!(
        fallback.normalized_visible_text.as_deref(),
        Some("[unsupported picture]")
    );
    let extracted_text = capture.document_ir.extracted_text();
    assert!(extracted_text.contains("[unsupported picture]"));
    for hidden in ["put", "framebox", "Should not render", "(0,0)", "(1,1)"] {
        assert!(!extracted_text.contains(hidden));
    }
    ...
    let display_list_text = capture.page_display_lists[0]
        ...
        .collect::<Vec<_>>()
        .join("");
    assert!(display_list_text.contains("[unsupported picture]"));
    for hidden in ["put", "framebox", "Should not render", "(0,0)", "(1,1)"] {
        assert!(!display_list_text.contains(hidden));
    }
}
...
fn captionbox_leading_optional_uses_long_caption_without_short_leakage() {
    let capture = capture_internal_render_ir(
        "main.tex",
        CAPTIONBOX_LEADING_OPTION_SOURCE,
        &SemanticAux::default(),
    );
    ...
    let extracted_text = capture.document_ir.extracted_text();
    assert!(extracted_text.contains("Subcaption long [?]."));
    assert!(extracted_text.contains("Captionbox long [?]."));
    for hidden in [
        "Subcaption short",
        "Captionbox short",
        "key",
...
const PICTURE_FALLBACK_SOURCE: &str = r"\begin{document}\begin{picture}(1,1)\put(0,0){\framebox(1,1){Should not render}}\end{picture}\end{document}";
...
const BRACED_LEGACY_EPSFIG_SOURCE: &str = r"\begin{document}\begin{figure}\epsfig{file={figures/plot},width=5cm}\caption{Plot caption.}\end{figure}\end{document}";
