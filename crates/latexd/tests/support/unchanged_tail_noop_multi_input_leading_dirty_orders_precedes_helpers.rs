enum LeadingDirtyOrdersPrecedesNoiseKind {
    Untracked,
    Unreadable,
}

enum LeadingDirtyOrdersPrecedesCase {
    Untracked,
    ReversedUntracked,
    Unreadable,
    ReversedUnreadable,
...
type LeadPreCase = LeadingDirtyOrdersPrecedesCase;

async fn run_lead_pre_case(case: LeadPreCase) {
    run_unchanged_tail_multi_input_leading_dirty_orders_precedes_case(case).await;
}

struct UnchangedTailMultiInputLeadingDirtyOrdersPrecedesFixture {
...
async fn prepare_unchanged_tail_multi_input_leading_dirty_orders_precedes_fixture()
-> UnchangedTailMultiInputLeadingDirtyOrdersPrecedesFixture {
...
fn rewrite_unchanged_tail_multi_input_leading_dirty_orders_precedes(
    fixture: &UnchangedTailMultiInputLeadingDirtyOrdersPrecedesFixture,
    noise_kind: LeadingDirtyOrdersPrecedesNoiseKind,
) {
    fs::write(
    ...
    match noise_kind {
        LeadingDirtyOrdersPrecedesNoiseKind::Untracked => {
            fs::write(fixture.root.join("notes.txt"), "scratch notes").expect("write notes");
        }
        LeadingDirtyOrdersPrecedesNoiseKind::Unreadable => {
...
async fn compile_unchanged_tail_multi_input_leading_dirty_orders_precedes_second_pass(
    fixture: &UnchangedTailMultiInputLeadingDirtyOrdersPrecedesFixture,
...
async fn run_unchanged_tail_multi_input_leading_dirty_orders_precedes_reuse(
    noise_kind: LeadingDirtyOrdersPrecedesNoiseKind,
    changed_files: &[&str],
) {
    let fixture = prepare_unchanged_tail_multi_input_leading_dirty_orders_precedes_fixture().await;
    rewrite_unchanged_tail_multi_input_leading_dirty_orders_precedes(&fixture, noise_kind);
    let changed_files = changed_files
        .iter()
        .map(|changed_file| Utf8PathBuf::from(*changed_file))
        .collect::<Vec<_>>();
    let second = compile_unchanged_tail_multi_input_leading_dirty_orders_precedes_second_pass(
        &fixture,
        &changed_files,
    )
    .await;
    assert_unchanged_tail_multi_input_leading_dirty_orders_precedes_reuse(
        &fixture,
        &second,
        &changed_files,
    );
...
async fn run_unchanged_tail_multi_input_leading_dirty_orders_precedes_case(
    case: LeadingDirtyOrdersPrecedesCase,
) {
    match case {
        LeadingDirtyOrdersPrecedesCase::Untracked => {
            run_unchanged_tail_multi_input_leading_dirty_orders_precedes_reuse(
                LeadingDirtyOrdersPrecedesNoiseKind::Untracked,
                &["notes.txt", "sections/body-a.tex", "sections/body-b.tex"],
            )
            .await;
        }
        LeadingDirtyOrdersPrecedesCase::ReversedUntracked => {
            run_unchanged_tail_multi_input_leading_dirty_orders_precedes_reuse(
                LeadingDirtyOrdersPrecedesNoiseKind::Untracked,
                &["notes.txt", "sections/body-b.tex", "sections/body-a.tex"],
            )
            .await;
        }
        LeadingDirtyOrdersPrecedesCase::Unreadable => {
            run_unchanged_tail_multi_input_leading_dirty_orders_precedes_reuse(
                LeadingDirtyOrdersPrecedesNoiseKind::Unreadable,
                &["notes.txt", "sections/body-a.tex", "sections/body-b.tex"],
            )
        ...
        LeadingDirtyOrdersPrecedesCase::ReversedUnreadable => {
            run_unchanged_tail_multi_input_leading_dirty_orders_precedes_reuse(
                LeadingDirtyOrdersPrecedesNoiseKind::Unreadable,
                &["notes.txt", "sections/body-b.tex", "sections/body-a.tex"],
            )
...
fn assert_unchanged_tail_multi_input_leading_dirty_orders_precedes_reuse(
    fixture: &UnchangedTailMultiInputLeadingDirtyOrdersPrecedesFixture,
