enum ToplevelTrailingInterleavedDirtyOrder {
    Interleaved,
    OtherInterleaved,
}

type ToplevelTrailingInterCase = ToplevelTrailingInterleavedDirtyOrder;

async fn run_toplevel_trailing_inter_case(case: ToplevelTrailingInterCase) {
    run_unchanged_tail_with_toplevel_trailing_interleaved_reuse(case).await;
}

struct UnchangedTailWithToplevelTrailingInterleavedFixture {
...
async fn prepare_unchanged_tail_with_toplevel_trailing_interleaved_fixture()
-> UnchangedTailWithToplevelTrailingInterleavedFixture {
...
fn rewrite_unchanged_tail_with_toplevel_trailing_interleaved(
    fixture: &UnchangedTailWithToplevelTrailingInterleavedFixture,
) {
    fs::write(
...
async fn run_unchanged_tail_with_toplevel_trailing_interleaved_reuse(
    dirty_order: ToplevelTrailingInterleavedDirtyOrder,
) {
    let fixture = prepare_unchanged_tail_with_toplevel_trailing_interleaved_fixture().await;
    rewrite_unchanged_tail_with_toplevel_trailing_interleaved(&fixture);
    let changed_files = match dirty_order {
        ToplevelTrailingInterleavedDirtyOrder::Interleaved => vec![
            Utf8PathBuf::from("sections/body-a.tex"),
            Utf8PathBuf::from("main.tex"),
            Utf8PathBuf::from("sections/body-b.tex"),
        ],
        ToplevelTrailingInterleavedDirtyOrder::OtherInterleaved => vec![
            Utf8PathBuf::from("sections/body-b.tex"),
            Utf8PathBuf::from("main.tex"),
            Utf8PathBuf::from("sections/body-a.tex"),
        ],
    };
    let second = compile_unchanged_tail_with_toplevel_trailing_interleaved_second_pass(
        &fixture,
        &changed_files,
    )
    .await;
    assert_unchanged_tail_with_toplevel_trailing_interleaved_reuse(
        &fixture,
        &second,
        &changed_files,
    );
...
async fn compile_unchanged_tail_with_toplevel_trailing_interleaved_second_pass(
    fixture: &UnchangedTailWithToplevelTrailingInterleavedFixture,
...
fn assert_unchanged_tail_with_toplevel_trailing_interleaved_reuse(
    fixture: &UnchangedTailWithToplevelTrailingInterleavedFixture,
