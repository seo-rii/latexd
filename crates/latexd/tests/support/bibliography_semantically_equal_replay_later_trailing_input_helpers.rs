struct LaterTrailingInputSemanticallyEqualReplayFixture {
...
enum LaterTrailingInputSemanticallyEqualReplayNoise {
    Untracked,
    Unreadable,
}

enum LaterTrailingInputSemanticallyEqualReplayStart {
    Preamble,
    Bibliography,
}

enum LaterTrailingInputSemanticallyEqualReplayOrder {
    Baseline,
    Reversed,
}

enum LaterTrailingInputSemanticallyEqualReplayCase {
    Baseline,
    Reversed,
    UntrackedFollows,
    UntrackedPrecedes,
...
type TrailSemEqCase = LaterTrailingInputSemanticallyEqualReplayCase;

async fn run_trail_sem_eq_case(case: TrailSemEqCase) {
    run_later_trailing_input_semantically_equal_replay_case(case).await;
}

async fn prepare_later_trailing_input_semantically_equal_replay_fixture()
-> LaterTrailingInputSemanticallyEqualReplayFixture {
...
"#,
    ...
    fs::write(
        root.join("main.tex"),
        "\\documentclass{article}\\begin{document}\\input{sections/body}\\bibliography{refs}\\input{sections/tail}\\end{document}",
    )
    .expect("write main");
    fs::write(
...
fn rewrite_later_trailing_input_semantically_equal_replay(
    fixture: &LaterTrailingInputSemanticallyEqualReplayFixture,
) {
    fs::write(
...
async fn compile_later_trailing_input_semantically_equal_replay_second_pass(
    fixture: &LaterTrailingInputSemanticallyEqualReplayFixture,
...
async fn run_later_trailing_input_semantically_equal_dirty_replay(
    noise: LaterTrailingInputSemanticallyEqualReplayNoise,
    dirty_files: &[&str],
    replay_start: LaterTrailingInputSemanticallyEqualReplayStart,
) {
    let fixture = prepare_later_trailing_input_semantically_equal_replay_fixture().await;
    rewrite_later_trailing_input_semantically_equal_replay(&fixture);
    match noise {
        LaterTrailingInputSemanticallyEqualReplayNoise::Untracked => {
            fs::write(fixture.root.join("notes.txt"), "scratch notes").expect("write notes");
        }
        LaterTrailingInputSemanticallyEqualReplayNoise::Unreadable => {
    ...
    let dirty_files = dirty_files
        ...
        .collect::<Vec<_>>();
    let second =
        compile_later_trailing_input_semantically_equal_replay_second_pass(&fixture, &dirty_files)
            .await;

    let (expected_checkpoint_id, expected_start_page_index) = match replay_start {
        LaterTrailingInputSemanticallyEqualReplayStart::Preamble => {
            (fixture.preamble_checkpoint_id.as_str(), 0)
        }
        LaterTrailingInputSemanticallyEqualReplayStart::Bibliography => (
            fixture.bibliography_checkpoint_id.as_str(),
            fixture.bibliography_checkpoint_page_index,
        ),
    };
    assert_later_trailing_input_semantically_equal_replay(
        &fixture,
        &second,
        &dirty_files,
...
async fn run_later_trailing_input_semantically_equal_baseline_replay(
    order: LaterTrailingInputSemanticallyEqualReplayOrder,
) {
    let fixture = prepare_later_trailing_input_semantically_equal_replay_fixture().await;
    rewrite_later_trailing_input_semantically_equal_replay(&fixture);
    let dirty_files = match order {
        LaterTrailingInputSemanticallyEqualReplayOrder::Baseline => vec![
            Utf8PathBuf::from("refs.bbl"),
            Utf8PathBuf::from("sections/tail.tex"),
        ],
        LaterTrailingInputSemanticallyEqualReplayOrder::Reversed => vec![
            Utf8PathBuf::from("sections/tail.tex"),
            Utf8PathBuf::from("refs.bbl"),
        ],
    };
...
async fn run_later_trailing_input_semantically_equal_replay_case(
    case: LaterTrailingInputSemanticallyEqualReplayCase,
) {
    match case {
        LaterTrailingInputSemanticallyEqualReplayCase::Baseline => {
            run_later_trailing_input_semantically_equal_baseline_replay(
                LaterTrailingInputSemanticallyEqualReplayOrder::Baseline,
            )
            .await;
        }
        LaterTrailingInputSemanticallyEqualReplayCase::Reversed => {
            run_later_trailing_input_semantically_equal_baseline_replay(
                LaterTrailingInputSemanticallyEqualReplayOrder::Reversed,
            )
            .await;
        }
        LaterTrailingInputSemanticallyEqualReplayCase::UntrackedFollows => {
            run_later_trailing_input_semantically_equal_dirty_replay(
                LaterTrailingInputSemanticallyEqualReplayNoise::Untracked,
        ...
        LaterTrailingInputSemanticallyEqualReplayCase::UntrackedPrecedes => {
            run_later_trailing_input_semantically_equal_dirty_replay(
                LaterTrailingInputSemanticallyEqualReplayNoise::Untracked,
        ...
        LaterTrailingInputSemanticallyEqualReplayCase::UnreadableFollows => {
            run_later_trailing_input_semantically_equal_dirty_replay(
                LaterTrailingInputSemanticallyEqualReplayNoise::Unreadable,
                &["refs.bbl", "sections/tail.tex", "notes.txt"],
                LaterTrailingInputSemanticallyEqualReplayStart::Preamble,
        ...
        LaterTrailingInputSemanticallyEqualReplayCase::UnreadablePrecedes => {
            run_later_trailing_input_semantically_equal_dirty_replay(
                LaterTrailingInputSemanticallyEqualReplayNoise::Unreadable,
                &["notes.txt", "refs.bbl", "sections/tail.tex"],
                LaterTrailingInputSemanticallyEqualReplayStart::Preamble,
...
fn assert_later_trailing_input_semantically_equal_replay(
    fixture: &LaterTrailingInputSemanticallyEqualReplayFixture,
