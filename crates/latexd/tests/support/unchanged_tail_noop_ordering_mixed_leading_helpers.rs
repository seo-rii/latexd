#[derive(Clone, Copy)]
enum OrderingMixedLeadingRewriteKind {
    InputLeading,
    ToplevelLeading,
}

enum OrderingMixedLeadingCase {
    InputLeading,
    ToplevelLeading,
    ReversedInputLeading,
    ReversedToplevelLeading,
}

type OrderMixedCase = OrderingMixedLeadingCase;

async fn run_order_mixed_case(case: OrderMixedCase) {
    run_unchanged_tail_ordering_mixed_leading_case(case).await;
}

struct UnchangedTailOrderingMixedLeadingFixture {
...
async fn prepare_unchanged_tail_ordering_mixed_leading_fixture()
-> UnchangedTailOrderingMixedLeadingFixture {
...
fn rewrite_unchanged_tail_ordering_mixed_leading(
    fixture: &UnchangedTailOrderingMixedLeadingFixture,
    rewrite_kind: OrderingMixedLeadingRewriteKind,
) {
    match rewrite_kind {
        OrderingMixedLeadingRewriteKind::InputLeading => {
            fs::write(
        ...
        OrderingMixedLeadingRewriteKind::ToplevelLeading => {
            fs::write(
                fixture.root.join("main.tex"),
...
async fn compile_unchanged_tail_ordering_mixed_leading_second_pass(
    fixture: &UnchangedTailOrderingMixedLeadingFixture,
...
fn assert_unchanged_tail_ordering_mixed_leading_reuse(
    fixture: &UnchangedTailOrderingMixedLeadingFixture,
    second: &CompileOutcome,
    dirty_files: &[Utf8PathBuf],
    expects_checkpoint_reuse: bool,
) {
...
async fn run_unchanged_tail_ordering_mixed_leading_reuse(
    rewrite_kind: OrderingMixedLeadingRewriteKind,
    dirty_files: &[&str],
) {
    let fixture = prepare_unchanged_tail_ordering_mixed_leading_fixture().await;
    rewrite_unchanged_tail_ordering_mixed_leading(&fixture, rewrite_kind);
    let dirty_files = dirty_files
        ...
        .collect::<Vec<_>>();
    let second =
        compile_unchanged_tail_ordering_mixed_leading_second_pass(&fixture, &dirty_files).await;
    assert_unchanged_tail_ordering_mixed_leading_reuse(
        &fixture,
        &second,
        &dirty_files,
        matches!(rewrite_kind, OrderingMixedLeadingRewriteKind::InputLeading),
    );
}

async fn run_unchanged_tail_ordering_mixed_leading_case(case: OrderingMixedLeadingCase) {
    match case {
        OrderingMixedLeadingCase::InputLeading => {
            run_unchanged_tail_ordering_mixed_leading_reuse(
                OrderingMixedLeadingRewriteKind::InputLeading,
                &["sections/body.tex", "main.tex"],
            )
            .await;
        }
        OrderingMixedLeadingCase::ToplevelLeading => {
            run_unchanged_tail_ordering_mixed_leading_reuse(
                OrderingMixedLeadingRewriteKind::ToplevelLeading,
                &["sections/body.tex", "main.tex"],
            )
        ...
        OrderingMixedLeadingCase::ReversedInputLeading => {
            run_unchanged_tail_ordering_mixed_leading_reuse(
                OrderingMixedLeadingRewriteKind::InputLeading,
        ...
        OrderingMixedLeadingCase::ReversedToplevelLeading => {
            run_unchanged_tail_ordering_mixed_leading_reuse(
                OrderingMixedLeadingRewriteKind::ToplevelLeading,
                &["main.tex", "sections/body.tex"],
            )
