struct SamePageIncludedBodyTrailingInputBaselineFixture {
...
#[derive(Clone, Copy)]
enum SamePageIncludedBodyTrailingInputBaselineCase {
    Baseline,
    Reversed,
    UntrackedFollows,
    UntrackedPrecedes,
...
type BodyTrailingBaseCase = SamePageIncludedBodyTrailingInputBaselineCase;

async fn run_body_trailing_base_case(case: BodyTrailingBaseCase) {
    run_same_page_included_body_trailing_input_baseline_replay(case).await;
}

async fn prepare_same_page_included_body_trailing_input_baseline_fixture()
-> SamePageIncludedBodyTrailingInputBaselineFixture {
...
fn rewrite_same_page_included_body_trailing_input_baseline(
    fixture: &SamePageIncludedBodyTrailingInputBaselineFixture,
) {
...
fn write_same_page_included_body_trailing_input_untracked_noise(root: &Utf8Path) {
    fs::write(root.join("notes.txt"), "scratch notes").expect("write notes");
}

fn write_same_page_included_body_trailing_input_unreadable_noise(root: &Utf8Path) {
    fs::create_dir_all(root.join("notes.txt")).expect("create unreadable dirty dir");
}

async fn compile_same_page_included_body_trailing_input_second_pass(
    fixture: &SamePageIncludedBodyTrailingInputBaselineFixture,
...
fn assert_same_page_included_body_trailing_input_replay(
    build_root: &Utf8Path,
...
async fn run_same_page_included_body_trailing_input_baseline_replay(
    case: SamePageIncludedBodyTrailingInputBaselineCase,
) {
    let fixture = prepare_same_page_included_body_trailing_input_baseline_fixture().await;
    rewrite_same_page_included_body_trailing_input_baseline(&fixture);
    match case {
        SamePageIncludedBodyTrailingInputBaselineCase::UntrackedFollows
        | SamePageIncludedBodyTrailingInputBaselineCase::UntrackedPrecedes => {
            write_same_page_included_body_trailing_input_untracked_noise(&fixture.root);
        }
        SamePageIncludedBodyTrailingInputBaselineCase::UnreadableFollows
        | SamePageIncludedBodyTrailingInputBaselineCase::UnreadablePrecedes => {
            write_same_page_included_body_trailing_input_unreadable_noise(&fixture.root);
        }
        SamePageIncludedBodyTrailingInputBaselineCase::Baseline
        | SamePageIncludedBodyTrailingInputBaselineCase::Reversed => {}
    }

    let dirty_files = match case {
        SamePageIncludedBodyTrailingInputBaselineCase::Baseline => vec![
            Utf8PathBuf::from("refsb.bbl"),
            Utf8PathBuf::from("refsa.bbl"),
            Utf8PathBuf::from("sections/tail.tex"),
        ],
        SamePageIncludedBodyTrailingInputBaselineCase::Reversed => vec![
            Utf8PathBuf::from("sections/tail.tex"),
            Utf8PathBuf::from("refsb.bbl"),
            Utf8PathBuf::from("refsa.bbl"),
        ],
        SamePageIncludedBodyTrailingInputBaselineCase::UntrackedFollows
        | SamePageIncludedBodyTrailingInputBaselineCase::UnreadableFollows => vec![
            Utf8PathBuf::from("refsb.bbl"),
            Utf8PathBuf::from("refsa.bbl"),
            Utf8PathBuf::from("sections/tail.tex"),
        ...
        SamePageIncludedBodyTrailingInputBaselineCase::UntrackedPrecedes
        | SamePageIncludedBodyTrailingInputBaselineCase::UnreadablePrecedes => vec![
            Utf8PathBuf::from("notes.txt"),
            Utf8PathBuf::from("refsb.bbl"),
            Utf8PathBuf::from("refsa.bbl"),
            Utf8PathBuf::from("sections/tail.tex"),
    ...
    let second =
        compile_same_page_included_body_trailing_input_second_pass(&fixture, &dirty_files).await;
    ...
    assert_same_page_included_body_trailing_input_replay(
        &fixture.build_root,
        &second,
        &dirty_files,
