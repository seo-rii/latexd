struct UnchangedTailMixedToplevelDirtyOrdersFixture {
...
#[derive(Clone, Copy)]
enum UnchangedTailMixedToplevelDirtyOrderVariant {
    Baseline,
    Reversed,
    UntrackedFollows,
    UnreadableFollows,
...
type MixedTopCase = UnchangedTailMixedToplevelDirtyOrderVariant;

async fn run_mixed_top_case(case: MixedTopCase) {
    run_unchanged_tail_mixed_toplevel_dirty_order_reuse(case).await;
}

async fn prepare_unchanged_tail_mixed_toplevel_dirty_orders_fixture()
-> UnchangedTailMixedToplevelDirtyOrdersFixture {
...
fn rewrite_unchanged_tail_mixed_toplevel_dirty_orders(
    fixture: &UnchangedTailMixedToplevelDirtyOrdersFixture,
) {
    fs::write(
...
fn write_unchanged_tail_mixed_toplevel_dirty_orders_untracked(root: &Utf8Path) {
    fs::write(root.join("notes.txt"), "scratch notes").expect("write notes");
}

fn write_unchanged_tail_mixed_toplevel_dirty_orders_unreadable(root: &Utf8Path) {
    fs::create_dir_all(root.join("notes.txt")).expect("create unreadable dirty dir");
}

async fn run_unchanged_tail_mixed_toplevel_dirty_order_reuse(
    dirty_order: UnchangedTailMixedToplevelDirtyOrderVariant,
) {
    let fixture = prepare_unchanged_tail_mixed_toplevel_dirty_orders_fixture().await;
    rewrite_unchanged_tail_mixed_toplevel_dirty_orders(&fixture);
    match dirty_order {
        UnchangedTailMixedToplevelDirtyOrderVariant::UntrackedFollows
        | UnchangedTailMixedToplevelDirtyOrderVariant::ReversedUntrackedFollows => {
            write_unchanged_tail_mixed_toplevel_dirty_orders_untracked(&fixture.root);
        }
        UnchangedTailMixedToplevelDirtyOrderVariant::UnreadableFollows
        | UnchangedTailMixedToplevelDirtyOrderVariant::ReversedUnreadableFollows => {
            write_unchanged_tail_mixed_toplevel_dirty_orders_unreadable(&fixture.root);
        }
        UnchangedTailMixedToplevelDirtyOrderVariant::Baseline
        | UnchangedTailMixedToplevelDirtyOrderVariant::Reversed => {}
    }

    let changed_files = match dirty_order {
        UnchangedTailMixedToplevelDirtyOrderVariant::Baseline => vec![
            Utf8PathBuf::from("sections/body-a.tex"),
            Utf8PathBuf::from("sections/body-b.tex"),
            Utf8PathBuf::from("main.tex"),
        ],
        UnchangedTailMixedToplevelDirtyOrderVariant::Reversed => vec![
            Utf8PathBuf::from("main.tex"),
            Utf8PathBuf::from("sections/body-b.tex"),
            Utf8PathBuf::from("sections/body-a.tex"),
        ],
        UnchangedTailMixedToplevelDirtyOrderVariant::UntrackedFollows
        | UnchangedTailMixedToplevelDirtyOrderVariant::UnreadableFollows => vec![
            Utf8PathBuf::from("sections/body-a.tex"),
            Utf8PathBuf::from("sections/body-b.tex"),
            Utf8PathBuf::from("main.tex"),
        ...
        UnchangedTailMixedToplevelDirtyOrderVariant::ReversedUntrackedFollows
        | UnchangedTailMixedToplevelDirtyOrderVariant::ReversedUnreadableFollows => vec![
            Utf8PathBuf::from("main.tex"),
            Utf8PathBuf::from("sections/body-b.tex"),
            Utf8PathBuf::from("sections/body-a.tex"),
            Utf8PathBuf::from("notes.txt"),
    ...
    let second =
        compile_unchanged_tail_mixed_toplevel_dirty_orders_second_pass(&fixture, &changed_files)
            .await;
    assert_unchanged_tail_mixed_toplevel_dirty_orders_reuse(&fixture, &second, &changed_files);
}

async fn compile_unchanged_tail_mixed_toplevel_dirty_orders_second_pass(
    fixture: &UnchangedTailMixedToplevelDirtyOrdersFixture,
...
fn assert_unchanged_tail_mixed_toplevel_dirty_orders_reuse(
    fixture: &UnchangedTailMixedToplevelDirtyOrdersFixture,
