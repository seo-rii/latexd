enum SamePageIncludedBodyTrailingInputSiblingExtraDirtyNoiseKind {
    Untracked,
    Unreadable,
}

enum SamePageIncludedBodyTrailingInputSiblingExtraDirtyReplayStart {
    Preamble,
    Bibliography,
}

enum SamePageIncludedBodyTrailingInputSiblingExtraDirtyCase {
    ...
    UnreadableDirtyFollows,
    UnreadableDirtyPrecedes,
...
type BodySiblingExtraCase = SamePageIncludedBodyTrailingInputSiblingExtraDirtyCase;

async fn run_body_sibling_extra_case(case: BodySiblingExtraCase) {
    run_same_page_included_body_trailing_input_sibling_extra_dirty_case(case).await;
}

struct SamePageIncludedBodyTrailingInputSiblingExtraDirtyFixture {
...
async fn prepare_same_page_included_body_trailing_input_sibling_extra_dirty_fixture()
-> SamePageIncludedBodyTrailingInputSiblingExtraDirtyFixture {
...
fn rewrite_same_page_included_body_trailing_input_sibling_extra_dirty(
    fixture: &SamePageIncludedBodyTrailingInputSiblingExtraDirtyFixture,
    noise_kind: SamePageIncludedBodyTrailingInputSiblingExtraDirtyNoiseKind,
) {
    ...
    match noise_kind {
        SamePageIncludedBodyTrailingInputSiblingExtraDirtyNoiseKind::Untracked => {
            fs::write(fixture.root.join("notes.txt"), "scratch notes").expect("write notes");
        }
        SamePageIncludedBodyTrailingInputSiblingExtraDirtyNoiseKind::Unreadable => {
...
async fn compile_same_page_included_body_trailing_input_sibling_extra_dirty_second_pass(
    fixture: &SamePageIncludedBodyTrailingInputSiblingExtraDirtyFixture,
...
async fn run_same_page_included_body_trailing_input_sibling_extra_dirty_replay(
    noise_kind: SamePageIncludedBodyTrailingInputSiblingExtraDirtyNoiseKind,
    dirty_files: &[&str],
    replay_start: SamePageIncludedBodyTrailingInputSiblingExtraDirtyReplayStart,
) {
    let fixture =
        prepare_same_page_included_body_trailing_input_sibling_extra_dirty_fixture().await;
    rewrite_same_page_included_body_trailing_input_sibling_extra_dirty(&fixture, noise_kind);

    let dirty_files = dirty_files
        .iter()
        .map(|dirty_file| Utf8PathBuf::from(*dirty_file))
        .collect::<Vec<_>>();
    let second = compile_same_page_included_body_trailing_input_sibling_extra_dirty_second_pass(
        &fixture,
        &dirty_files,
    )
    .await;
    ...
    let (expected_checkpoint_id, expected_page_index) = match replay_start {
        SamePageIncludedBodyTrailingInputSiblingExtraDirtyReplayStart::Preamble => (
            fixture.preamble_checkpoint_id.as_str(),
            fixture.preamble_page_index_after,
        ),
        SamePageIncludedBodyTrailingInputSiblingExtraDirtyReplayStart::Bibliography => (
            fixture.bibliography_checkpoint_id.as_str(),
            fixture.bibliography_page_index_after,
        ),
    };
    assert_same_page_included_body_trailing_input_sibling_extra_dirty_replay(
        &fixture.build_root,
        &second,
        &dirty_files,
...
async fn run_same_page_included_body_trailing_input_sibling_extra_dirty_case(
    case: SamePageIncludedBodyTrailingInputSiblingExtraDirtyCase,
) {
    let (noise_kind, dirty_files, replay_start) = match case {
        SamePageIncludedBodyTrailingInputSiblingExtraDirtyCase::UntrackedDirtyFollows => (
            SamePageIncludedBodyTrailingInputSiblingExtraDirtyNoiseKind::Untracked,
            &["refsb.bbl", "sections/tail.tex", "refsa.bbl", "notes.txt"][..],
            SamePageIncludedBodyTrailingInputSiblingExtraDirtyReplayStart::Preamble,
        ),
        SamePageIncludedBodyTrailingInputSiblingExtraDirtyCase::UntrackedDirtyPrecedes => (
            SamePageIncludedBodyTrailingInputSiblingExtraDirtyNoiseKind::Untracked,
            &["notes.txt", "sections/tail.tex", "refsa.bbl", "refsb.bbl"][..],
            SamePageIncludedBodyTrailingInputSiblingExtraDirtyReplayStart::Bibliography,
        ),
        SamePageIncludedBodyTrailingInputSiblingExtraDirtyCase::UntrackedInterleavedFollows => (
            SamePageIncludedBodyTrailingInputSiblingExtraDirtyNoiseKind::Untracked,
            &["sections/tail.tex", "refsb.bbl", "refsa.bbl", "notes.txt"][..],
            SamePageIncludedBodyTrailingInputSiblingExtraDirtyReplayStart::Preamble,
        ),
        SamePageIncludedBodyTrailingInputSiblingExtraDirtyCase::UntrackedInterleavedPrecedes => (
            SamePageIncludedBodyTrailingInputSiblingExtraDirtyNoiseKind::Untracked,
            &["notes.txt", "sections/tail.tex", "refsa.bbl", "refsb.bbl"][..],
            SamePageIncludedBodyTrailingInputSiblingExtraDirtyReplayStart::Bibliography,
        ),
        SamePageIncludedBodyTrailingInputSiblingExtraDirtyCase::UntrackedOtherInterleavedFollows => (
            SamePageIncludedBodyTrailingInputSiblingExtraDirtyNoiseKind::Untracked,
            &["sections/tail.tex", "refsa.bbl", "refsb.bbl", "notes.txt"][..],
            SamePageIncludedBodyTrailingInputSiblingExtraDirtyReplayStart::Preamble,
        ),
        SamePageIncludedBodyTrailingInputSiblingExtraDirtyCase::UntrackedOtherInterleavedPrecedes => (
            SamePageIncludedBodyTrailingInputSiblingExtraDirtyNoiseKind::Untracked,
            &["notes.txt", "sections/tail.tex", "refsa.bbl", "refsb.bbl"][..],
            SamePageIncludedBodyTrailingInputSiblingExtraDirtyReplayStart::Bibliography,
        ),
        SamePageIncludedBodyTrailingInputSiblingExtraDirtyCase::UnreadableDirtyFollows => (
            SamePageIncludedBodyTrailingInputSiblingExtraDirtyNoiseKind::Unreadable,
            &["refsb.bbl", "sections/tail.tex", "refsa.bbl", "notes.txt"][..],
            SamePageIncludedBodyTrailingInputSiblingExtraDirtyReplayStart::Preamble,
        ),
        SamePageIncludedBodyTrailingInputSiblingExtraDirtyCase::UnreadableDirtyPrecedes => (
            SamePageIncludedBodyTrailingInputSiblingExtraDirtyNoiseKind::Unreadable,
            &["notes.txt", "sections/tail.tex", "refsa.bbl", "refsb.bbl"][..],
            SamePageIncludedBodyTrailingInputSiblingExtraDirtyReplayStart::Preamble,
        ),
        SamePageIncludedBodyTrailingInputSiblingExtraDirtyCase::UnreadableInterleavedFollows => (
            SamePageIncludedBodyTrailingInputSiblingExtraDirtyNoiseKind::Unreadable,
            &["sections/tail.tex", "refsb.bbl", "refsa.bbl", "notes.txt"][..],
            SamePageIncludedBodyTrailingInputSiblingExtraDirtyReplayStart::Preamble,
        ),
        SamePageIncludedBodyTrailingInputSiblingExtraDirtyCase::UnreadableInterleavedPrecedes => (
            SamePageIncludedBodyTrailingInputSiblingExtraDirtyNoiseKind::Unreadable,
            &["notes.txt", "sections/tail.tex", "refsa.bbl", "refsb.bbl"][..],
            SamePageIncludedBodyTrailingInputSiblingExtraDirtyReplayStart::Preamble,
        ),
        SamePageIncludedBodyTrailingInputSiblingExtraDirtyCase::UnreadableOtherInterleavedFollows => (
            SamePageIncludedBodyTrailingInputSiblingExtraDirtyNoiseKind::Unreadable,
            &["sections/tail.tex", "refsa.bbl", "refsb.bbl", "notes.txt"][..],
            SamePageIncludedBodyTrailingInputSiblingExtraDirtyReplayStart::Preamble,
        ),
        SamePageIncludedBodyTrailingInputSiblingExtraDirtyCase::UnreadableOtherInterleavedPrecedes => (
            SamePageIncludedBodyTrailingInputSiblingExtraDirtyNoiseKind::Unreadable,
            &["notes.txt", "sections/tail.tex", "refsa.bbl", "refsb.bbl"][..],
            SamePageIncludedBodyTrailingInputSiblingExtraDirtyReplayStart::Preamble,
        ),
    };
    run_same_page_included_body_trailing_input_sibling_extra_dirty_replay(
        noise_kind,
        dirty_files,
        replay_start,
    )
...
fn assert_same_page_included_body_trailing_input_sibling_extra_dirty_replay(
    build_root: &Utf8Path,
