struct UnchangedTailMultiInputDirtyOrdersPrecedesFixture {
...
enum UnchangedTailMultiInputDirtyOrdersPrecedesNoiseKind {
    Untracked,
    Unreadable,
}

type InputPreCase = UnchangedTailMultiInputDirtyOrdersPrecedesNoiseKind;

async fn run_input_pre_case(case: InputPreCase) {
    run_unchanged_tail_multi_input_dirty_orders_precedes_noise(case).await;
}

async fn prepare_unchanged_tail_multi_input_dirty_orders_precedes_fixture()
-> UnchangedTailMultiInputDirtyOrdersPrecedesFixture {
...
fn rewrite_unchanged_tail_multi_input_dirty_orders_precedes(
    fixture: &UnchangedTailMultiInputDirtyOrdersPrecedesFixture,
) {
    fs::write(
...
fn write_unchanged_tail_multi_input_dirty_orders_precedes_untracked(root: &Utf8Path) {
    fs::write(root.join("notes.txt"), "scratch notes").expect("write notes");
}

fn write_unchanged_tail_multi_input_dirty_orders_precedes_unreadable(root: &Utf8Path) {
    fs::create_dir_all(root.join("notes.txt")).expect("create unreadable dirty dir");
}

async fn compile_unchanged_tail_multi_input_dirty_orders_precedes_second_pass(
    fixture: &UnchangedTailMultiInputDirtyOrdersPrecedesFixture,
...
fn assert_unchanged_tail_multi_input_dirty_orders_precedes_reuse(
    fixture: &UnchangedTailMultiInputDirtyOrdersPrecedesFixture,
...
async fn run_unchanged_tail_multi_input_dirty_orders_precedes_noise(
    noise_kind: UnchangedTailMultiInputDirtyOrdersPrecedesNoiseKind,
) {
    let fixture = prepare_unchanged_tail_multi_input_dirty_orders_precedes_fixture().await;
    rewrite_unchanged_tail_multi_input_dirty_orders_precedes(&fixture);
    match noise_kind {
        UnchangedTailMultiInputDirtyOrdersPrecedesNoiseKind::Untracked => {
            write_unchanged_tail_multi_input_dirty_orders_precedes_untracked(&fixture.root);
        }
        UnchangedTailMultiInputDirtyOrdersPrecedesNoiseKind::Unreadable => {
            write_unchanged_tail_multi_input_dirty_orders_precedes_unreadable(&fixture.root);
        }
    }
    ...
    let second = compile_unchanged_tail_multi_input_dirty_orders_precedes_second_pass(
        &fixture,
        &changed_files,
    )
    .await;
    assert_unchanged_tail_multi_input_dirty_orders_precedes_reuse(
        &fixture,
        &second,
        &changed_files,
    );
