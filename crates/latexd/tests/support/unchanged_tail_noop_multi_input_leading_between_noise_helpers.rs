enum LeadingBetweenNoiseKind {
    Untracked,
    Unreadable,
}

enum LeadingBetweenNoiseDirtyOrder {
    Interleaved,
    OtherInterleaved,
}

enum LeadingBetweenNoiseCase {
    InterleavedUntracked,
    OtherInterleavedUntracked,
    InterleavedUnreadable,
    OtherInterleavedUnreadable,
}

type LeadBetweenCase = LeadingBetweenNoiseCase;

async fn run_lead_between_case(case: LeadBetweenCase) {
    run_unchanged_tail_multi_input_leading_between_noise_case(case).await;
}

struct UnchangedTailMultiInputLeadingBetweenNoiseFixture {
...
async fn prepare_unchanged_tail_multi_input_leading_between_noise_fixture()
-> UnchangedTailMultiInputLeadingBetweenNoiseFixture {
...
fn rewrite_unchanged_tail_multi_input_leading_between_noise(
    fixture: &UnchangedTailMultiInputLeadingBetweenNoiseFixture,
    noise_kind: LeadingBetweenNoiseKind,
) {
    fs::write(
...
async fn run_unchanged_tail_multi_input_leading_between_noise_reuse(
    noise_kind: LeadingBetweenNoiseKind,
    dirty_order: LeadingBetweenNoiseDirtyOrder,
) {
    let fixture = prepare_unchanged_tail_multi_input_leading_between_noise_fixture().await;
    rewrite_unchanged_tail_multi_input_leading_between_noise(&fixture, noise_kind);
    let changed_files = match dirty_order {
        LeadingBetweenNoiseDirtyOrder::Interleaved => vec![
            Utf8PathBuf::from("sections/body-a.tex"),
            Utf8PathBuf::from("notes.txt"),
            Utf8PathBuf::from("sections/body-b.tex"),
        ],
        LeadingBetweenNoiseDirtyOrder::OtherInterleaved => vec![
            Utf8PathBuf::from("sections/body-b.tex"),
            Utf8PathBuf::from("notes.txt"),
            Utf8PathBuf::from("sections/body-a.tex"),
        ],
    };
    let second = compile_unchanged_tail_multi_input_leading_between_noise_second_pass(
        &fixture,
        &changed_files,
    )
    .await;
    assert_unchanged_tail_multi_input_leading_between_noise_reuse(
        &fixture,
        &second,
        &changed_files,
    );
...
async fn run_unchanged_tail_multi_input_leading_between_noise_case(case: LeadingBetweenNoiseCase) {
    let (noise_kind, dirty_order) = match case {
        LeadingBetweenNoiseCase::InterleavedUntracked => (
            LeadingBetweenNoiseKind::Untracked,
            LeadingBetweenNoiseDirtyOrder::Interleaved,
        ),
        LeadingBetweenNoiseCase::OtherInterleavedUntracked => (
            LeadingBetweenNoiseKind::Untracked,
            LeadingBetweenNoiseDirtyOrder::OtherInterleaved,
        ),
        LeadingBetweenNoiseCase::InterleavedUnreadable => (
            LeadingBetweenNoiseKind::Unreadable,
            LeadingBetweenNoiseDirtyOrder::Interleaved,
        ),
        LeadingBetweenNoiseCase::OtherInterleavedUnreadable => (
            LeadingBetweenNoiseKind::Unreadable,
            LeadingBetweenNoiseDirtyOrder::OtherInterleaved,
        ),
    };

    run_unchanged_tail_multi_input_leading_between_noise_reuse(noise_kind, dirty_order).await;
}

async fn compile_unchanged_tail_multi_input_leading_between_noise_second_pass(
    fixture: &UnchangedTailMultiInputLeadingBetweenNoiseFixture,
...
fn assert_unchanged_tail_multi_input_leading_between_noise_reuse(
    fixture: &UnchangedTailMultiInputLeadingBetweenNoiseFixture,
