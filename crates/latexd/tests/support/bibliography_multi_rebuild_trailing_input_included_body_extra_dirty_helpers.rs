enum BibliographyMultiRebuildTrailingInputIncludedBodyExtraDirtyNoiseKind {
    Untracked,
    Unreadable,
}

enum BibliographyMultiRebuildTrailingInputIncludedBodyExtraDirtyCase {
    UntrackedFollows,
    UntrackedPrecedes,
    UnreadableFollows,
    UnreadablePrecedes,
...
struct BibliographyMultiRebuildTrailingInputIncludedBodyExtraDirtyFixture {
...
async fn prepare_bibliography_multi_rebuild_trailing_input_included_body_extra_dirty_fixture()
-> BibliographyMultiRebuildTrailingInputIncludedBodyExtraDirtyFixture {
...
fn rewrite_bibliography_multi_rebuild_trailing_input_included_body_extra_dirty(
    fixture: &BibliographyMultiRebuildTrailingInputIncludedBodyExtraDirtyFixture,
    noise_kind: BibliographyMultiRebuildTrailingInputIncludedBodyExtraDirtyNoiseKind,
) {
    fs::write(
    ...
    match noise_kind {
        BibliographyMultiRebuildTrailingInputIncludedBodyExtraDirtyNoiseKind::Untracked => {
            fs::write(fixture.root.join("notes.txt"), "scratch notes").expect("write notes");
        }
        BibliographyMultiRebuildTrailingInputIncludedBodyExtraDirtyNoiseKind::Unreadable => {
...
async fn compile_bibliography_multi_rebuild_trailing_input_included_body_extra_dirty_second_pass(
    fixture: &BibliographyMultiRebuildTrailingInputIncludedBodyExtraDirtyFixture,
...
async fn run_bibliography_multi_rebuild_trailing_input_included_body_extra_dirty_rebuild(
    noise_kind: BibliographyMultiRebuildTrailingInputIncludedBodyExtraDirtyNoiseKind,
    dirty_files: &[&str],
) {
    let fixture =
        prepare_bibliography_multi_rebuild_trailing_input_included_body_extra_dirty_fixture().await;
    rewrite_bibliography_multi_rebuild_trailing_input_included_body_extra_dirty(
        &fixture, noise_kind,
    );
    ...
    let dirty_files = dirty_files
        ...
        .collect::<Vec<_>>();
    let second =
        compile_bibliography_multi_rebuild_trailing_input_included_body_extra_dirty_second_pass(
            &fixture,
            &dirty_files,
        )
        .await;

    assert_bibliography_multi_rebuild_trailing_input_included_body_extra_dirty_rebuilds_from_base(
        &fixture,
        &second,
        &dirty_files,
    );
...
async fn run_bibliography_multi_rebuild_trailing_input_included_body_extra_dirty_case(
    case: BibliographyMultiRebuildTrailingInputIncludedBodyExtraDirtyCase,
) {
    let (noise_kind, dirty_files) = match case {
        BibliographyMultiRebuildTrailingInputIncludedBodyExtraDirtyCase::UntrackedFollows => (
            BibliographyMultiRebuildTrailingInputIncludedBodyExtraDirtyNoiseKind::Untracked,
            ["refsb.bbl", "sections/tail.tex", "notes.txt"],
        ),
        BibliographyMultiRebuildTrailingInputIncludedBodyExtraDirtyCase::UntrackedPrecedes => (
            BibliographyMultiRebuildTrailingInputIncludedBodyExtraDirtyNoiseKind::Untracked,
            ["notes.txt", "sections/tail.tex", "refsb.bbl"],
        ),
        BibliographyMultiRebuildTrailingInputIncludedBodyExtraDirtyCase::UnreadableFollows => (
            BibliographyMultiRebuildTrailingInputIncludedBodyExtraDirtyNoiseKind::Unreadable,
            ["refsb.bbl", "sections/tail.tex", "notes.txt"],
        ),
        BibliographyMultiRebuildTrailingInputIncludedBodyExtraDirtyCase::UnreadablePrecedes => (
            BibliographyMultiRebuildTrailingInputIncludedBodyExtraDirtyNoiseKind::Unreadable,
            ["notes.txt", "sections/tail.tex", "refsb.bbl"],
        ),
    };

    run_bibliography_multi_rebuild_trailing_input_included_body_extra_dirty_rebuild(
        noise_kind,
        &dirty_files,
    )
    .await;
...
type BodyTrailExtra = BibliographyMultiRebuildTrailingInputIncludedBodyExtraDirtyCase;

async fn run_body_trail_extra(case: BodyTrailExtra) {
    run_bibliography_multi_rebuild_trailing_input_included_body_extra_dirty_case(case).await;
}

fn assert_bibliography_multi_rebuild_trailing_input_included_body_extra_dirty_rebuilds_from_base(
    fixture: &BibliographyMultiRebuildTrailingInputIncludedBodyExtraDirtyFixture,
