enum OrderingDoubleLeadingOtherInterleavedNoiseKind {
    Untracked,
    Unreadable,
}

type OrderDoubleOtherCase = OrderingDoubleLeadingOtherInterleavedNoiseKind;

async fn run_order_double_other_case(case: OrderDoubleOtherCase) {
    run_unchanged_tail_ordering_double_leading_other_interleaved_reuse(case).await;
}

struct UnchangedTailOrderingDoubleLeadingOtherInterleavedFixture {
...
async fn prepare_unchanged_tail_ordering_double_leading_other_interleaved_fixture()
-> UnchangedTailOrderingDoubleLeadingOtherInterleavedFixture {
...
fn rewrite_unchanged_tail_ordering_double_leading_other_interleaved(
    fixture: &UnchangedTailOrderingDoubleLeadingOtherInterleavedFixture,
    noise_kind: OrderingDoubleLeadingOtherInterleavedNoiseKind,
) {
    fs::write(
    ...
    match noise_kind {
        OrderingDoubleLeadingOtherInterleavedNoiseKind::Untracked => {
            fs::write(fixture.root.join("notes.txt"), "scratch notes").expect("write notes");
        }
        OrderingDoubleLeadingOtherInterleavedNoiseKind::Unreadable => {
...
async fn compile_unchanged_tail_ordering_double_leading_other_interleaved_second_pass(
    fixture: &UnchangedTailOrderingDoubleLeadingOtherInterleavedFixture,
...
async fn run_unchanged_tail_ordering_double_leading_other_interleaved_reuse(
    noise_kind: OrderingDoubleLeadingOtherInterleavedNoiseKind,
) {
    let fixture = prepare_unchanged_tail_ordering_double_leading_other_interleaved_fixture().await;
    rewrite_unchanged_tail_ordering_double_leading_other_interleaved(&fixture, noise_kind);
    let dirty_files = vec![
        Utf8PathBuf::from("main.tex"),
        Utf8PathBuf::from("notes.txt"),
        Utf8PathBuf::from("sections/body.tex"),
    ];
    let second = compile_unchanged_tail_ordering_double_leading_other_interleaved_second_pass(
        &fixture,
        &dirty_files,
    )
    .await;
    assert_unchanged_tail_ordering_double_leading_other_interleaved_reuse(
        &fixture,
        &second,
        &dirty_files,
    );
...
fn assert_unchanged_tail_ordering_double_leading_other_interleaved_reuse(
    fixture: &UnchangedTailOrderingDoubleLeadingOtherInterleavedFixture,
