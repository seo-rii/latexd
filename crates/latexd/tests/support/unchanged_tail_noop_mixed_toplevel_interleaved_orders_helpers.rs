struct UnchangedTailMixedToplevelInterleavedOrdersFixture {
...
#[derive(Clone, Copy)]
enum UnchangedTailMixedToplevelInterleavedDirtyOrder {
    Base,
    UntrackedFollows,
    UntrackedPrecedes,
    UnreadableFollows,
...
type MixedTopInterCase = UnchangedTailMixedToplevelInterleavedDirtyOrder;

async fn run_mixed_top_inter_case(case: MixedTopInterCase) {
    run_unchanged_tail_mixed_toplevel_interleaved_reuse(case).await;
}

async fn prepare_unchanged_tail_mixed_toplevel_interleaved_fixture()
-> UnchangedTailMixedToplevelInterleavedOrdersFixture {
...
fn rewrite_unchanged_tail_mixed_toplevel_interleaved(
    fixture: &UnchangedTailMixedToplevelInterleavedOrdersFixture,
) {
    fs::write(
...
fn write_unchanged_tail_mixed_toplevel_interleaved_untracked(root: &Utf8Path) {
    fs::write(root.join("notes.txt"), "scratch notes").expect("write notes");
}

fn write_unchanged_tail_mixed_toplevel_interleaved_unreadable(root: &Utf8Path) {
    fs::create_dir_all(root.join("notes.txt")).expect("create unreadable dirty dir");
}

async fn run_unchanged_tail_mixed_toplevel_interleaved_reuse(
    dirty_order: UnchangedTailMixedToplevelInterleavedDirtyOrder,
) {
    let fixture = prepare_unchanged_tail_mixed_toplevel_interleaved_fixture().await;
    rewrite_unchanged_tail_mixed_toplevel_interleaved(&fixture);
    match dirty_order {
        UnchangedTailMixedToplevelInterleavedDirtyOrder::UntrackedFollows
        | UnchangedTailMixedToplevelInterleavedDirtyOrder::UntrackedPrecedes => {
            write_unchanged_tail_mixed_toplevel_interleaved_untracked(&fixture.root);
        }
        UnchangedTailMixedToplevelInterleavedDirtyOrder::UnreadableFollows
        | UnchangedTailMixedToplevelInterleavedDirtyOrder::UnreadablePrecedes => {
            write_unchanged_tail_mixed_toplevel_interleaved_unreadable(&fixture.root);
        }
        UnchangedTailMixedToplevelInterleavedDirtyOrder::Base => {}
    }

    let changed_files = match dirty_order {
        UnchangedTailMixedToplevelInterleavedDirtyOrder::Base => vec![
            Utf8PathBuf::from("sections/body-a.tex"),
            Utf8PathBuf::from("main.tex"),
            Utf8PathBuf::from("sections/body-b.tex"),
        ],
        UnchangedTailMixedToplevelInterleavedDirtyOrder::UntrackedFollows
        | UnchangedTailMixedToplevelInterleavedDirtyOrder::UnreadableFollows => vec![
            Utf8PathBuf::from("sections/body-a.tex"),
            Utf8PathBuf::from("main.tex"),
            Utf8PathBuf::from("sections/body-b.tex"),
        ...
        UnchangedTailMixedToplevelInterleavedDirtyOrder::UntrackedPrecedes
        | UnchangedTailMixedToplevelInterleavedDirtyOrder::UnreadablePrecedes => vec![
            Utf8PathBuf::from("notes.txt"),
            Utf8PathBuf::from("sections/body-a.tex"),
            Utf8PathBuf::from("main.tex"),
            Utf8PathBuf::from("sections/body-b.tex"),
    ...
    let second =
        compile_unchanged_tail_mixed_toplevel_interleaved_second_pass(&fixture, &changed_files)
            .await;
    assert_unchanged_tail_mixed_toplevel_interleaved_reuse(&fixture, &second, &changed_files);
}

async fn compile_unchanged_tail_mixed_toplevel_interleaved_second_pass(
    fixture: &UnchangedTailMixedToplevelInterleavedOrdersFixture,
...
fn assert_unchanged_tail_mixed_toplevel_interleaved_reuse(
    fixture: &UnchangedTailMixedToplevelInterleavedOrdersFixture,
