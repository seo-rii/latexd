enum MixedBetweenNoiseKind {
    Untracked,
    Unreadable,
}

enum MixedBetweenNoiseDirtyOrder {
    Interleaved,
    OtherInterleaved,
}

enum MixedBetweenNoiseCase {
...
type MixBetweenCase = MixedBetweenNoiseCase;

async fn run_mix_between_case(case: MixBetweenCase) {
    run_unchanged_tail_multi_input_mixed_between_noise_case(case).await;
}

struct UnchangedTailMultiInputMixedBetweenNoiseFixture {
...
async fn prepare_unchanged_tail_multi_input_mixed_between_noise_fixture()
-> UnchangedTailMultiInputMixedBetweenNoiseFixture {
...
fn rewrite_unchanged_tail_multi_input_mixed_between_noise(
    fixture: &UnchangedTailMultiInputMixedBetweenNoiseFixture,
    noise_kind: MixedBetweenNoiseKind,
) {
    fs::write(
...
async fn run_unchanged_tail_multi_input_mixed_between_noise_reuse(
    noise_kind: MixedBetweenNoiseKind,
    dirty_order: MixedBetweenNoiseDirtyOrder,
) {
    let fixture = prepare_unchanged_tail_multi_input_mixed_between_noise_fixture().await;
    rewrite_unchanged_tail_multi_input_mixed_between_noise(&fixture, noise_kind);
    let changed_files = match dirty_order {
        MixedBetweenNoiseDirtyOrder::Interleaved => vec![
            Utf8PathBuf::from("sections/body-a.tex"),
            Utf8PathBuf::from("notes.txt"),
            Utf8PathBuf::from("sections/body-b.tex"),
        ],
        MixedBetweenNoiseDirtyOrder::OtherInterleaved => vec![
            Utf8PathBuf::from("sections/body-b.tex"),
            Utf8PathBuf::from("notes.txt"),
            Utf8PathBuf::from("sections/body-a.tex"),
        ],
    };
    let second = compile_unchanged_tail_multi_input_mixed_between_noise_second_pass(
        &fixture,
        &changed_files,
    )
    .await;
    assert_unchanged_tail_multi_input_mixed_between_noise_reuse(&fixture, &second, &changed_files);
}

async fn run_unchanged_tail_multi_input_mixed_between_noise_case(case: MixedBetweenNoiseCase) {
    let (noise_kind, dirty_order) = match case {
        MixedBetweenNoiseCase::InterleavedUntracked => (
            MixedBetweenNoiseKind::Untracked,
            MixedBetweenNoiseDirtyOrder::Interleaved,
        ),
        MixedBetweenNoiseCase::OtherInterleavedUntracked => (
            MixedBetweenNoiseKind::Untracked,
            MixedBetweenNoiseDirtyOrder::OtherInterleaved,
        ),
        MixedBetweenNoiseCase::InterleavedUnreadable => (
            MixedBetweenNoiseKind::Unreadable,
            MixedBetweenNoiseDirtyOrder::Interleaved,
        ),
        MixedBetweenNoiseCase::OtherInterleavedUnreadable => (
            MixedBetweenNoiseKind::Unreadable,
            MixedBetweenNoiseDirtyOrder::OtherInterleaved,
        ),
    };

    run_unchanged_tail_multi_input_mixed_between_noise_reuse(noise_kind, dirty_order).await;
}

async fn compile_unchanged_tail_multi_input_mixed_between_noise_second_pass(
    fixture: &UnchangedTailMultiInputMixedBetweenNoiseFixture,
...
fn assert_unchanged_tail_multi_input_mixed_between_noise_reuse(
    fixture: &UnchangedTailMultiInputMixedBetweenNoiseFixture,
