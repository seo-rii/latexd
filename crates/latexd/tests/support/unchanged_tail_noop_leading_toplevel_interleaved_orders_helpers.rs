struct UnchangedTailLeadingToplevelInterleavedFixture {
...
#[derive(Clone, Copy)]
enum UnchangedTailLeadingToplevelInterleavedDirtyOrder {
    Base,
    UntrackedFollows,
    UntrackedPrecedes,
    UnreadableFollows,
...
type LeadTopInterCase = UnchangedTailLeadingToplevelInterleavedDirtyOrder;

async fn run_lead_top_inter_case(case: LeadTopInterCase) {
    run_unchanged_tail_leading_toplevel_interleaved_reuse(case).await;
}

async fn prepare_unchanged_tail_leading_toplevel_interleaved_fixture()
-> UnchangedTailLeadingToplevelInterleavedFixture {
...
fn rewrite_unchanged_tail_leading_toplevel_interleaved(
    fixture: &UnchangedTailLeadingToplevelInterleavedFixture,
) {
    fs::write(
...
fn write_unchanged_tail_leading_toplevel_interleaved_untracked(root: &Utf8Path) {
    fs::write(root.join("notes.txt"), "scratch notes").expect("write notes");
}

fn write_unchanged_tail_leading_toplevel_interleaved_unreadable(root: &Utf8Path) {
    fs::create_dir_all(root.join("notes.txt")).expect("create unreadable dirty dir");
}

async fn compile_unchanged_tail_leading_toplevel_interleaved_second_pass(
    fixture: &UnchangedTailLeadingToplevelInterleavedFixture,
...
fn assert_unchanged_tail_leading_toplevel_interleaved_reuse(
    fixture: &UnchangedTailLeadingToplevelInterleavedFixture,
...
async fn run_unchanged_tail_leading_toplevel_interleaved_reuse(
    dirty_order: UnchangedTailLeadingToplevelInterleavedDirtyOrder,
) {
    let fixture = prepare_unchanged_tail_leading_toplevel_interleaved_fixture().await;
    rewrite_unchanged_tail_leading_toplevel_interleaved(&fixture);

    match dirty_order {
        UnchangedTailLeadingToplevelInterleavedDirtyOrder::UntrackedFollows
        | UnchangedTailLeadingToplevelInterleavedDirtyOrder::UntrackedPrecedes => {
            write_unchanged_tail_leading_toplevel_interleaved_untracked(&fixture.root);
        }
        UnchangedTailLeadingToplevelInterleavedDirtyOrder::UnreadableFollows
        | UnchangedTailLeadingToplevelInterleavedDirtyOrder::UnreadablePrecedes => {
            write_unchanged_tail_leading_toplevel_interleaved_unreadable(&fixture.root);
        }
        UnchangedTailLeadingToplevelInterleavedDirtyOrder::Base => {}
    }

    let changed_files = match dirty_order {
        UnchangedTailLeadingToplevelInterleavedDirtyOrder::Base => vec![
            Utf8PathBuf::from("sections/body-a.tex"),
            Utf8PathBuf::from("main.tex"),
            Utf8PathBuf::from("sections/body-b.tex"),
        ],
        UnchangedTailLeadingToplevelInterleavedDirtyOrder::UntrackedPrecedes
        | UnchangedTailLeadingToplevelInterleavedDirtyOrder::UnreadablePrecedes => vec![
        ...
        UnchangedTailLeadingToplevelInterleavedDirtyOrder::UntrackedFollows
        | UnchangedTailLeadingToplevelInterleavedDirtyOrder::UnreadableFollows => vec![
            Utf8PathBuf::from("sections/body-a.tex"),
            Utf8PathBuf::from("main.tex"),
            Utf8PathBuf::from("sections/body-b.tex"),
            Utf8PathBuf::from("notes.txt"),
    ...
    let second =
        compile_unchanged_tail_leading_toplevel_interleaved_second_pass(&fixture, &changed_files)
            .await;
    assert_unchanged_tail_leading_toplevel_interleaved_reuse(&fixture, &second, &changed_files);
}
