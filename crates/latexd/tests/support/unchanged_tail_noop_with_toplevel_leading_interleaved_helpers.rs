enum ToplevelLeadingInterleavedDirtyOrder {
    Interleaved,
    OtherInterleaved,
}

type ToplevelLeadingInterCase = ToplevelLeadingInterleavedDirtyOrder;

async fn run_toplevel_leading_inter_case(case: ToplevelLeadingInterCase) {
    run_unchanged_tail_with_toplevel_leading_interleaved_reuse(case).await;
}

struct UnchangedTailWithToplevelLeadingInterleavedFixture {
...
async fn prepare_unchanged_tail_with_toplevel_leading_interleaved_fixture()
-> UnchangedTailWithToplevelLeadingInterleavedFixture {
...
fn rewrite_unchanged_tail_with_toplevel_leading_interleaved(
    fixture: &UnchangedTailWithToplevelLeadingInterleavedFixture,
) {
    fs::write(
...
async fn run_unchanged_tail_with_toplevel_leading_interleaved_reuse(
    dirty_order: ToplevelLeadingInterleavedDirtyOrder,
) {
    let fixture = prepare_unchanged_tail_with_toplevel_leading_interleaved_fixture().await;
    rewrite_unchanged_tail_with_toplevel_leading_interleaved(&fixture);
    let changed_files = match dirty_order {
        ToplevelLeadingInterleavedDirtyOrder::Interleaved => vec![
            Utf8PathBuf::from("sections/body-a.tex"),
            Utf8PathBuf::from("main.tex"),
            Utf8PathBuf::from("sections/body-b.tex"),
        ],
        ToplevelLeadingInterleavedDirtyOrder::OtherInterleaved => vec![
            Utf8PathBuf::from("sections/body-b.tex"),
            Utf8PathBuf::from("main.tex"),
            Utf8PathBuf::from("sections/body-a.tex"),
        ],
    };
    let second = compile_unchanged_tail_with_toplevel_leading_interleaved_second_pass(
        &fixture,
        &changed_files,
    )
    .await;
    assert_unchanged_tail_with_toplevel_leading_interleaved_reuse(
        &fixture,
        &second,
        &changed_files,
    );
...
async fn compile_unchanged_tail_with_toplevel_leading_interleaved_second_pass(
    fixture: &UnchangedTailWithToplevelLeadingInterleavedFixture,
...
fn assert_unchanged_tail_with_toplevel_leading_interleaved_reuse(
    fixture: &UnchangedTailWithToplevelLeadingInterleavedFixture,
