enum BetweenNoiseUntrackedDirtyOrder {
    Interleaved,
    OtherInterleaved,
}

type InputBetweenUntrackedCase = BetweenNoiseUntrackedDirtyOrder;

async fn run_input_between_untracked_case(case: InputBetweenUntrackedCase) {
    run_unchanged_tail_multi_input_between_noise_untracked_reuse(case).await;
}

struct UnchangedTailMultiInputBetweenNoiseUntrackedFixture {
...
async fn prepare_unchanged_tail_multi_input_between_noise_untracked_fixture()
-> UnchangedTailMultiInputBetweenNoiseUntrackedFixture {
...
fn rewrite_unchanged_tail_multi_input_between_noise_untracked(
    fixture: &UnchangedTailMultiInputBetweenNoiseUntrackedFixture,
) {
    fs::write(
...
async fn run_unchanged_tail_multi_input_between_noise_untracked_reuse(
    dirty_order: BetweenNoiseUntrackedDirtyOrder,
) {
    let fixture = prepare_unchanged_tail_multi_input_between_noise_untracked_fixture().await;
    rewrite_unchanged_tail_multi_input_between_noise_untracked(&fixture);
    let changed_files = match dirty_order {
        BetweenNoiseUntrackedDirtyOrder::Interleaved => vec![
            Utf8PathBuf::from("sections/body-a.tex"),
            Utf8PathBuf::from("notes.txt"),
            Utf8PathBuf::from("sections/body-b.tex"),
        ],
        BetweenNoiseUntrackedDirtyOrder::OtherInterleaved => vec![
            Utf8PathBuf::from("sections/body-b.tex"),
            Utf8PathBuf::from("notes.txt"),
            Utf8PathBuf::from("sections/body-a.tex"),
        ],
    };
    let second = compile_unchanged_tail_multi_input_between_noise_untracked_second_pass(
        &fixture,
        &changed_files,
    )
    .await;
    assert_unchanged_tail_multi_input_between_noise_untracked_reuse(
        &fixture,
        &second,
        &changed_files,
    );
...
async fn compile_unchanged_tail_multi_input_between_noise_untracked_second_pass(
    fixture: &UnchangedTailMultiInputBetweenNoiseUntrackedFixture,
...
fn assert_unchanged_tail_multi_input_between_noise_untracked_reuse(
    fixture: &UnchangedTailMultiInputBetweenNoiseUntrackedFixture,
