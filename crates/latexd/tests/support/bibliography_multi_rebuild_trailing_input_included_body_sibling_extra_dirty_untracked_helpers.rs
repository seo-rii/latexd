struct BibliographyMultiRebuildTrailingInputIncludedBodySiblingExtraDirtyUntrackedFixture {
...
enum SiblingExtraDirtyUntrackedOrder {
    Follows,
    Precedes,
}

async fn prepare_bibliography_multi_rebuild_trailing_input_included_body_sibling_extra_dirty_untracked_fixture()
-> BibliographyMultiRebuildTrailingInputIncludedBodySiblingExtraDirtyUntrackedFixture {
...
fn rewrite_bibliography_multi_rebuild_trailing_input_included_body_sibling_extra_dirty_untracked(
    fixture: &BibliographyMultiRebuildTrailingInputIncludedBodySiblingExtraDirtyUntrackedFixture,
) {
    fs::write(
...
async fn run_sibling_extra_dirty_untracked_rebuild(order: SiblingExtraDirtyUntrackedOrder) {
    let fixture =
        prepare_bibliography_multi_rebuild_trailing_input_included_body_sibling_extra_dirty_untracked_fixture()
            .await;
    rewrite_bibliography_multi_rebuild_trailing_input_included_body_sibling_extra_dirty_untracked(
        &fixture,
    );
    fs::write(fixture.root.join("notes.txt"), "scratch notes").expect("write notes");

    let dirty_files = match order {
        SiblingExtraDirtyUntrackedOrder::Follows => {
            ["sections/tail.tex", "refsa.bbl", "refsb.bbl", "notes.txt"]
        }
        SiblingExtraDirtyUntrackedOrder::Precedes => {
    ...
    let dirty_files = dirty_files
        ...
        .collect::<Vec<_>>();
    let second =
        compile_bibliography_multi_rebuild_trailing_input_included_body_sibling_extra_dirty_untracked_second_pass(
            &fixture,
            &dirty_files,
        )
        .await;

    assert_bibliography_multi_rebuild_trailing_input_included_body_sibling_extra_dirty_untracked_rebuilds_from_base(
        &fixture,
        &second,
        &dirty_files,
    );
...
async fn compile_bibliography_multi_rebuild_trailing_input_included_body_sibling_extra_dirty_untracked_second_pass(
    fixture: &BibliographyMultiRebuildTrailingInputIncludedBodySiblingExtraDirtyUntrackedFixture,
...
fn assert_bibliography_multi_rebuild_trailing_input_included_body_sibling_extra_dirty_untracked_rebuilds_from_base(
    fixture: &BibliographyMultiRebuildTrailingInputIncludedBodySiblingExtraDirtyUntrackedFixture,
