use tex_tfm_metrics::dimension_subset::{
    ExactTfmDimensions, ExtractError, ScaleError, extract_exact_frame,
};

const CMR10: &[u8] = include_bytes!("../../tex-fonts/assets/classic/tfm/cmr10.tfm");
const CMR7: &[u8] = include_bytes!("../../tex-fonts/assets/classic/tfm/cmr7.tfm");

#[test]
fn computer_modern_dimension_metrics_match_the_native_oracle_exactly() {
    let cmr10 = extract_exact_frame(CMR10).expect("cmr10 must expose exact dimension metrics");
    assert_eq!(cmr10.design_size_sp(), 10 * 65_536);
    assert_eq!(
        cmr10.at_size_sp(cmr10.design_size_sp()).unwrap().quad_sp,
        655_361
    );
    assert_eq!(
        cmr10
            .at_size_sp(cmr10.design_size_sp())
            .unwrap()
            .x_height_sp,
        282_168
    );
    assert_eq!(cmr10.at_size_sp(12 * 65_536).unwrap().quad_sp, 786_434);
    assert_eq!(cmr10.at_size_sp(12 * 65_536).unwrap().x_height_sp, 338_602);

    let cmr7 = extract_exact_frame(CMR7).expect("cmr7 must expose exact dimension metrics");
    assert_eq!(cmr7.design_size_sp(), 7 * 65_536);
    assert_eq!(
        cmr7.at_size_sp(cmr7.design_size_sp()).unwrap().quad_sp,
        522_469
    );
    assert_eq!(
        cmr7.at_size_sp(cmr7.design_size_sp()).unwrap().x_height_sp,
        197_518
    );
}

#[test]
fn content_identity_is_tfm_only_and_byte_stable() {
    let first = extract_exact_frame(CMR10).unwrap();
    let second = extract_exact_frame(CMR10).unwrap();
    let distinct = extract_exact_frame(CMR7).unwrap();

    assert_eq!(
        first.exact_frame_content_hash(),
        second.exact_frame_content_hash()
    );
    assert_ne!(
        first.exact_frame_content_hash(),
        distinct.exact_frame_content_hash()
    );
    assert_eq!(
        first.exact_frame_content_hash(),
        "sha256:87f2d8981927644cbecaf3d639e96e348ea4e7be49d8804468bd8ba9ff3f5244"
    );
    assert_eq!(
        distinct.exact_frame_content_hash(),
        "sha256:28f9f4d237bdae4babef4a59d90f17686a09d6f9df0b2c3c23b2ba15f20eee82"
    );
}

#[test]
fn malformed_or_incomplete_tfm_data_fails_without_a_fallback() {
    assert_eq!(extract_exact_frame(&[0; 23]), Err(ExtractError::TooShort));

    let mut bad_length = CMR10.to_vec();
    bad_length[0..2].copy_from_slice(&1u16.to_be_bytes());
    assert!(matches!(
        extract_exact_frame(&bad_length),
        Err(ExtractError::ExactFrameLengthMismatch { .. })
    ));

    let mut zero_design_size = CMR10.to_vec();
    zero_design_size[28..32].fill(0);
    assert_eq!(
        extract_exact_frame(&zero_design_size),
        Err(ExtractError::InvalidDesignSize)
    );
}

#[test]
fn valid_short_parameter_tables_zero_fill_missing_dimensions() {
    let mut missing_quad = CMR10[..CMR10.len() - 8].to_vec();
    missing_quad[0..2].copy_from_slice(&322u16.to_be_bytes());
    missing_quad[22..24].copy_from_slice(&5u16.to_be_bytes());
    let metrics =
        extract_exact_frame(&missing_quad).expect("np=5 exposes a zero-filled exact subset");
    assert_eq!(
        metrics.at_size_sp(metrics.design_size_sp()).unwrap(),
        ExactTfmDimensions {
            quad_sp: 0,
            x_height_sp: 282_168,
        }
    );

    let mut missing_both = CMR10[..CMR10.len() - 12].to_vec();
    missing_both[0..2].copy_from_slice(&321u16.to_be_bytes());
    missing_both[22..24].copy_from_slice(&4u16.to_be_bytes());
    let metrics =
        extract_exact_frame(&missing_both).expect("np=4 exposes a zero-filled exact subset");
    assert_eq!(
        metrics.at_size_sp(metrics.design_size_sp()).unwrap(),
        ExactTfmDimensions {
            quad_sp: 0,
            x_height_sp: 0,
        }
    );
}

#[test]
fn invalid_or_overflowing_effective_sizes_are_typed_errors() {
    let metrics = extract_exact_frame(CMR10).unwrap();

    assert_eq!(metrics.at_size_sp(0), Err(ScaleError::NonPositiveSize));
    assert_eq!(metrics.at_size_sp(i32::MAX), Err(ScaleError::Overflow));
}

#[test]
fn scaling_matches_tex82_at_negative_and_large_size_boundaries() {
    let cmr10 = extract_exact_frame(CMR10).unwrap();
    assert_eq!(cmr10.at_size_sp((1 << 23) + 1).unwrap().quad_sp, 8_388_632);

    let mut negative_x_height = CMR10.to_vec();
    let counts = (0..12)
        .map(|index| u16::from_be_bytes([CMR10[index * 2], CMR10[index * 2 + 1]]) as usize)
        .collect::<Vec<_>>();
    let [_, lh, bc, ec, nw, nh, nd, ni, nl, nk, ne, _] = counts.as_slice() else {
        unreachable!()
    };
    let character_count = ec - bc + 1;
    let parameter_start = 4 * (6 + lh + character_count + nw + nh + nd + ni + nl + nk + ne);
    negative_x_height[parameter_start + 16..parameter_start + 20]
        .copy_from_slice(&(-1i32).to_be_bytes());

    let metrics = extract_exact_frame(&negative_x_height).unwrap();
    assert_eq!(metrics.at_size_sp(1).unwrap().x_height_sp, -1);
}
