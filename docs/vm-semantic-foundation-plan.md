...
The broad `new()` compatibility path remains until each producer family
declares its origin at the emission boundary, so this contract is only partial.
...
| primitive/macro origin | migrated VM families promote executed events to `Primitive` or `Macro`; executed list items and environment boundaries now pass that origin into construction | partial | migrate every remaining emission family and remove the broad default constructor path |
| explicit constructor contract | `with_origin()` requires producer/confidence and has a focused model test; executed list items and environment boundaries use it, while `new()` remains a broad compatibility default | partial | migrate all production call sites, then remove or restrict the compatibility constructor |
