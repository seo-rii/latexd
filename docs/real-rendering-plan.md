...
The same `href`-first rule applies to PDF vector path-like and image `<use>`
references when both attributes point at conflicting definitions.
...
- project-root display-list SVG debug artifacts can embed resolver-backed SVG
  ...
  and `<defs><image>` definitions reused through `<use>`,
  `href` priority over `xlink:href` for conflicting PDF vector path-like and
  image `<use>` references,
