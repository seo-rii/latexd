...
* M13 업데이트: 위 M12 완료 시점의 서술 중 `\long`/`\protected`/`\outer`가 no-op이라는 부분은 더 이상 현재 상태가 아니다. full parameter text와 delimited argument, `\long` paragraph policy, `\protected` full-expansion deferral, `\outer` forbidden scanner contexts가 snapshot/`\ifx` identity와 함께 구현돼 있다. 다만 prefix scanner의 command-class 검증, structured diagnostic, expandable/unexpandable command split은 아직 남아 있다.
...
**상태:** `2026-07-28` 기준 구현 진행 중.

현재까지 streaming Mouth, Eqtb/SaveStack의 주요 assignment class,
scanner producer/confidence, replay-safe semantic event prefix, full macro
parameter text와 prefix 의미, 그리고 여러 execution-authoritative event
family가 들어갔다. 남은 핵심은 old scope/assignment 제거, file-aware token
origin과 stable event identity, command/nest 통합, scanner-only family 이전,
완전한 continuation snapshot이다.
