# Rust LaTeX HMR 컴파일러 설계 초안

이 문서는 **“대부분의 arXiv 논문이 돌아가는 LaTeX 호환 Rust 컴파일러 + 웹 프리뷰 HMR 시스템”**을 구현하기 위한 청사진이다. 기준 호환 프로파일은 arXiv Submission 1.5의 현재 동작으로 잡는다. arXiv는 현재 TeX Live 2023과 2025를 지원하고, `pdflatex`/`latex+dvips_ps2pdf`/`xelatex` 같은 명시적인 컴파일 경로를 사용하며, 컴파일은 **항상 submission root에서** 수행된다. 또 `00README`로 `compiler`, `sources`, `toplevel`, `texlive_version` 등을 고정한다. 따라서 엔진도 처음부터 추상적인 “LaTeX 호환”이 아니라 **`ArxivTL2025` / `ArxivTL2023` 프로파일**을 가지는 쪽이 맞다. 1차 목표는 `pdflatex` 우선 경로로 자르고, `latex+dvips_ps2pdf`와 `xelatex`는 후순위로 두는 것이 현실적이다. ([arXiv][1])

이 설계는 네 가지 기존 구현에서 배운다. Tectonic은 support files와 format을 캐시하고, TeX 단계를 자동으로 재실행해 출력이 안정될 때까지 돌며, 의존성 규칙도 내보낼 수 있다. Typst는 `World` 추상화 위에서 증분 파서, 모듈/클로저 메모이제이션, 요소 단위 레이아웃 캐시를 사용한다. Tinymist는 **웹 프리뷰가 PDF 프리뷰보다 빠르며** background preview server와 partial rendering이 실용적임을 보여준다. Ghostscript는 `display_update`와 `display_rectangle_request` 콜백으로 page/rectangle 단위 렌더링을 가능하게 하지만, `gsapi_*()` 호출은 한 스레드에서만 해야 하고, 투명도를 쓰는 파일에서는 `display_update`가 아예 오지 않을 수 있다. 이 조합이 바로 “백엔드 HMR + 프론트엔드 HMR” 설계의 출발점이다. ([Tectonic][2])

---

## 0. 현재 구현 상태

이 문서는 여전히 **설계/로드맵 문서**다. 다만 현재 저장소에는 이미 상당 부분의 세로 슬라이스가 구현되어 있으므로, 읽는 사람이 설계와 구현 상태를 혼동하지 않도록 현재 시점을 명시해 둔다.

2026-03-27 기준 상태:

* `M0`~`M9`는 bootstrap 수준을 넘어서 실제 코드와 회귀 테스트가 들어가 있다.
* `M10`은 현재 README 기준 완료 상태다. 브라우저 프리뷰 클릭, source/preview jump, source pane, URL hash deep-link, local editor bridge, line-aware sync box까지 연결되어 있다.
* `M11`은 이제 `M11.1`~`M11.4`로 쪼개어 관리하고, 현재 저장소 기준으로는 완료 상태다. semantic aux artifact, rerun/backdating, executed-source rewrite, checkpoint/page reuse가 모두 regression에 올라가 있다.
* `M12`도 이제 `M12.1`~`M12.6`으로 쪼개어 관리하고, 현재 저장소 기준으로는 완료 상태다. wrapper-heavy corpus, bibliography/toolchain hardening, structured failure coverage, renderer/session, sync/editor hardening까지 현재 definition of done을 충족한 상태로 본다.
* 현재 구현 상세는 `PROGRESS.md`를 최신 기준으로 본다. README는 “어디로 가야 하는가”, `PROGRESS.md`는 “지금 어디까지 왔는가”를 설명한다.

즉, 아래 milestone들은 순수한 미래 계획만은 아니고, 일부는 이미 코드로 올라와 있으며, 일부는 여전히 설계 단계에 남아 있다.
...
### TDD가 아닌 contract/smoke로 갈 컴포넌트
...
이 8개가 끝나면 **프론트/백 HMR UX는 이미 존재**하고, 그 아래 코어만 Rust 엔진으로 바꿔 나가면 된다.
...
6. **arXiv corpus 없이 개발하는 것**
   실제 source+PDF pair가 있어야 어디까지 왔는지 알 수 있다. arXiv는 S3 source/PDF와 OAI-PMH/API metadata를 제공한다. ([arXiv][13])
...
원하면 다음 답변에서는 이 초안을 바로 코드 수준으로 내려서 **`Cargo workspace`, 핵심 trait, SQLite 스키마, WebSocket 메시지 정의, 첫 3개 PR의 디렉터리/파일 생성 순서**까지 써드리겠다.
